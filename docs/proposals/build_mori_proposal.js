...
/* ============================ 4. 提携全体像 ============================ */
{
  ...
  const progs = [
    ...
    { n: "03", t: "WineBank CLUB ヒルズ会員", d: "レストランでワインが仕入原価相当。通常プランに加えヒルズ向けのオーダーメイド版をご提案。", tag: "ワーカー／レジデンス" },
...
/* ============================ 9. WineBank CLUB ============================ */
{
  const s = slide("PROGRAM 03-A", "WineBank CLUB（通常）",
    "2025年7月に開始した月額会員制サービスです（旧：Fine Wine Collection）。レストランでワインを飲食店仕入れ原価相当で楽しめる点が中核です。");
  ...
  const perks = [
    ["レストランでワインが約半額", "飲食店の仕入れ原価相当でご提供。一般の飲食店は卸原価の2〜3倍が通例です。"],
    ["コルケージ完全無料", "提携店へのBYOで抜栓料・持込料ゼロ。WineGOで購入した場合は最短5分で「手ぶらBYO」。"],
    ["同伴者も同じ特典", "会員ご本人だけでなく、同席の方も同じ価格でお楽しみいただけます。"],
    ["ワイン配送 月1〜2回無料", "自宅・会食先・贈答先へ。都心はWineGOで当日デリバリー。"],
    ["ネット最安値水準でのワイン購入", "インポーター直の仕入力による特別価格。会員プランに応じて設定。"],
  ];
  perks.forEach((p, i) => {
    const y = 2.26 + i * 0.66;
    s.addShape(pres.ShapeType.ellipse, {
  ...
  bullets(s, [
    "個人会員：月会費 8,800円〜（プランにより特典が異なります）",
    "入会・本人確認・請求はすべて当社が運用",
    "会員数は千人規模への拡大を計画",
...
/* ============================ 9b. Hills WineBank CLUB ============================ */
{
  ...
  const rows = [
    ...
    ["会費", "月会費 8,800円〜", "ヒルズ枠として特別設定。法人は人数枠課金"],
    ["レストラン", "ワインが仕入原価相当（約半額）", "館内の対象店を追加。同伴者も同特典"],
    ["コルケージ", "提携店へのBYO無料", "館内対象店を追加。会食先への事前配送込み"],
    ["配送", "月1〜2回無料", "館内ストレージ開設後は即日・回数を拡大"],
  ...
  s.addText("会費・特典水準は、対象人数と館内対象店の範囲が決まった段階で確定させていただきます。特典の相当額の考え方は Appendix に記載しています。", {
...
/* ============================ 13. ポジショニング ============================ */
{
  ...
  const rw = W - M - rx;
  const points = [
    { n: "01", t: "会食のたびに、得をする", d: "コルケージ無料と仕入原価相当での提供。卸原価の2〜3倍を払う店内価格から解放されます。" },
...
/* ============================ 20b. 未来図 ============================ */
{
  const s = slide("THE FUTURE", "2030年のヒルズ ─ ワインが巡る街",
    "いまは各店がそれぞれ在庫を抱え、居住者は街の外で買い、ワーカーは店内価格を払っています。それが、こうなります。");

  // 中央ハブ
  const hx = 4.86, hy = 3.24, hw = 3.60, hh = 1.26;
  // つなぎ線（ハブ角 → 各カード内側角）
  const links = [
    { x: 4.10, y: 2.88, w: 0.76, h: 0.36, flipV: false },
    { x: hx + hw, y: 2.88, w: 0.76, h: 0.36, flipV: true },
    { x: 4.10, y: hy + hh, w: 0.76, h: 0.36, flipV: true },
    { x: hx + hw, y: hy + hh, w: 0.76, h: 0.36, flipV: false },
  ];
  links.forEach((l) => {
    s.addShape(pres.ShapeType.line, {
      x: l.x, y: l.y, w: l.w, h: l.h, flipV: l.flipV,
      line: { color: TAN, width: 1.5 },
    });
  });

  s.addShape(pres.ShapeType.roundRect, {
    x: hx, y: hy, w: hw, h: hh, rectRadius: 0.05,
    fill: { color: NAVY }, line: { width: 0 },
  });
  s.addText("街に、在庫はひとつだけ", {
    x: hx, y: hy + 0.14, w: hw, h: 0.24, margin: 0, align: "center",
    fontFace: FJP, fontSize: 9.5, bold: true, color: TAN,
  });
  s.addText("館内ワインストレージ ＋ WineGO", {
    x: hx, y: hy + 0.40, w: hw, h: 0.34, margin: 0, align: "center",
    fontFace: FJP, fontSize: 15, bold: true, color: PAPER,
  });
  s.addText("数千〜1万本を街区内に常備し、定温のまま即日・最短5分で動く。", {
    x: hx + 0.24, y: hy + 0.76, w: hw - 0.48, h: 0.40, margin: 0, align: "center",
    fontFace: FJP, fontSize: 9.2, color: "B9BEC2", lineSpacing: 12.5,
  });

  const nodes = [
    { x: 0.80, y: 1.68, tag: "レジデンス居住者",
      t: "自宅のセラーが、街の中にある",
      d: "買う・預ける・出すが一つの画面で完結。飲みたい夜に、飲みたい一本が届く。" },
    { x: 9.22, y: 1.68, tag: "商業区画",
      t: "街の目的地になる",
      d: "数千本のウォークインセラーとミシュランシェフ。ワインを目的に人が集まる場所へ。" },
    { x: 0.80, y: 4.86, tag: "テナント飲食店",
      t: "在庫ゼロで、世界水準のリスト",
      d: "ワイン在庫は0円。セラーは小さく、客席は広く。それでも品揃えは数万本。" },
    { x: 9.22, y: 4.86, tag: "ヒルズワーカー",
      t: "手ぶらで、会食へ",
      d: "仕入原価相当のワインをコルケージなしで。接待の質は上がり、費用は下がる。" },
  ];
  nodes.forEach((n) => {
    card(s, n.x, n.y, 3.30, 1.20);
    s.addText(n.tag, {
      x: n.x + 0.26, y: n.y + 0.14, w: 2.78, h: 0.22, margin: 0,
      fontFace: FJP, fontSize: 9, bold: true, color: TAN_D,
    });
    s.addText(n.t, {
      x: n.x + 0.26, y: n.y + 0.38, w: 2.78, h: 0.30, margin: 0,
      fontFace: FJP, fontSize: 12.5, bold: true, color: INK,
    });
    s.addText(n.d, {
      x: n.x + 0.26, y: n.y + 0.70, w: 2.78, h: 0.42, margin: 0,
      fontFace: FJP, fontSize: 9, color: GREY, lineSpacing: 12.5,
    });
  });

  // 中央左右の余白に、街の変化を一言ずつ
  s.addText("各店が抱えていた在庫が、\n街のひとつのセラーに集まる。", {
    x: 0.80, y: 3.34, w: 3.10, h: 0.70, margin: 0,
    fontFace: FJP, fontSize: 10.2, color: GREY, lineSpacing: 15,
  });
  s.addText("そのセラーから、\n街のあらゆる場所へワインが動く。", {
    x: 9.42, y: 3.34, w: 3.10, h: 0.70, margin: 0, align: "right",
    fontFace: FJP, fontSize: 10.2, color: GREY, lineSpacing: 15,
  });

  s.addShape(pres.ShapeType.rect, {
    x: M, y: 6.24, w: W - M * 2, h: 0.52, fill: { color: NAVY }, line: { width: 0 },
  });
  s.addText("他の街には真似のできない「ワインが巡る街」を、ヒルズに。", {
    x: M, y: 6.24, w: W - M * 2, h: 0.52, margin: 0, align: "center", valign: "middle",
    fontFace: FJP, fontSize: 15, bold: true, color: TAN_LT,
  });
}
...
/* ============================ 28. CLUB特典の価値 ============================ */
{
  const s = slide("APPENDIX", "リニューアル版 WineBank CLUB 会員特典の内訳（想定）",
    "現行は月会費8,800円〜のプラン制です。下表はワイン預り100本を含む上位プランを想定した提供価値の内訳で、ヒルズ枠を設計する際の基準としてご参照ください。");

  const rows = [
    ["サービス", "内容", "相当額"],
    ["初回引取・入庫・撮影・登録", "現在の保管先・ご自宅へ伺い、全ボトルを撮影・登録", "100,000円"],
    ["ワイン保管（100本まで）", "定温14℃・湿度70%の専門倉庫でお預かり", "120,000円"],
    ["会員資格（レストラン特典）", "ワインが仕入原価相当。同伴者も同特典", "100,000円"],
    ["出庫・ご配送", "回数無制限・無料（持込・贈答・ご自宅）", "54,000円"],
    ["コルケージ無料", "提携店へのBYO。抜栓料・持込料ゼロ", "60,000円"],
  ...
  s.addText("一般の飲食店はワインを卸原価の2〜3倍で提供しています。仕入原価相当での提供とBYO無料により、年間接待費600万円規模のご利用で約200万円の削減余地が生まれます。", {
    x: rx + 0.28, y: 2.24, w: rw - 0.56, h: 1.06, margin: 0,
    fontFace: FJP, fontSize: 10, color: "DDE1E4", lineSpacing: 15,
  });
  s.addText("特典46.4万円相当 ＋ 接待費削減 約200万円\n＝ 会費を大きく上回る実利", {
