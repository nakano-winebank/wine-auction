...
const TIER_TXT = "6ヶ月以内70% ／ 12ヶ月以内60% ／ 18ヶ月未満50% ／ 24ヶ月未満40% ／ 24ヶ月〜30%";
...
// ═══════════════════════════════════════════════════════ 02 サマリー
{
  ...
  const pts = [
    ...
    ["販売は委託販売。貴社が酒類の販売当事者にならない設計とする。",
     "WineBankが受託者として自己の名をもって販売し（問屋型）、所有権は貴社から顧客へ直接移転する。酒類販売業免許はWineBankが保有するものを用いる。"],
...
// ═══════════════════════════════════════════════════ 03 スキーム
{
  ...
  const boxes = [
    ...
    ["③ 販売（委託）", "貴社 → 顧客",
     ["WineBankが受託者として自己の名で販売", "所有権は貴社から顧客へ直接移転", "B2B酒販店卸とB2C自社EC・オークション"],
     "販売差益をティアで配分", false],
  ];
  boxes.forEach((b, i) => {
    const x = M + (bw + 0.34) * i;
  ...
  const rows = [
    ...
    [td("投資家の立場"), td("匿名組合員（出資者）"), td("現物の所有者", { color: GOLD_L, bold: true })],
  ...
  table(s, rows, M, 4.42, CW, [2.60, 4.65, 4.64], { rowH: 0.32, fontSize: 9 });

  note(s, "※ 取得のみ売買とし、以後の顧客への販売は委託販売とする。貴社が反復継続して酒類を販売する形にならないよう、WineBankが自己の名をもって販売する問屋型の受託とする。契約書の作成前に所轄税務署の酒類指導官および顧問弁護士の確認を経て確定させる。", 6.44);
...
// ═══════════════════════════════════════════ 04 なぜ委託販売か
{
  ...
  const items = [
    ["01", "酒類販売業免許", "酒類を継続反復して販売するには販売場ごとの免許が必要となる。貴社が現物を所有したうえで売却を繰り返す形にすると、貴社側に免許が必要となるおそれがある。"],
  ...
  s.addText("取得は売買、販売は委託。WineBankが自己の名をもって販売する問屋型の受託とする。", {
    x: M + 0.30, y: 4.20, w: CW - 0.60, h: 0.30, margin: 0,
    fontFace: SANS, fontSize: 14, bold: true, color: IVORY,
  });
  const two = [
    ["所有と販売を分ける理由",
     "貴社は現物の所有者にとどまり、販売の当事者にはならない。WineBankが受託者として自己の名をもって顧客へ販売し、所有権のみが貴社から顧客へ直接移転する。免許・取引口座・割当はすべてWineBank側に残るため、貴社側に免許取得の負担が生じない。"],
  ...
  note(s, "※ 委託販売の形式（問屋型か代理型か）、および貴社側に酒類販売業免許を要しないことの確認は、契約書ドラフト作成前に所轄税務署の酒類指導官および顧問弁護士との間で確定させる。移転価格については譲渡価格の算定根拠を文書化し、第三者卸価格との比較資料を年次で整備する。", 6.10);
...
// ═══════════════════════════════════════════ 09 利益配分ティア
{
  ...
  const tw = (CW - 0.24 * 4) / 5;
  tiers.forEach((t, i) => {
    const x = M + (tw + 0.24) * i;
    card(s, x, 1.80, tw, 1.62, { fill: t[3] ? CARD2 : CARD, line: t[3] ? GOLD_D : LINE });
    s.addText("在庫回転 " + t[0], {
      x: x + 0.18, y: 1.92, w: tw - 0.36, h: 0.24, margin: 0, align: "center",
      fontFace: SANS, fontSize: 10, bold: true, color: t[3] ? GOLD_L : IVORY,
    });
    s.addText("WineBank", {
      x: x + 0.18, y: 2.22, w: tw - 0.36, h: 0.20, margin: 0, align: "center",
      fontFace: SANS, fontSize: 8.5, color: MUTED,
    });
    s.addText(t[1], {
      x: x + 0.18, y: 2.40, w: tw - 0.36, h: 0.40, margin: 0, align: "center",
      fontFace: LATIN, fontSize: 24, bold: true, color: IVORY,
    });
    s.addText("投資家 " + t[2], {
      x: x + 0.18, y: 2.86, w: tw - 0.36, h: 0.26, margin: 0, align: "center",
      fontFace: SANS, fontSize: 11, bold: true, color: GOLD_L,
    });
  });
  ...
  table(s, rows, M + hw + 0.30, 3.62, hw, [1.42, 1.02, 0.94, 1.32, 1.10], { rowH: 0.44, fontSize: 9 });
...
// ═══════════════════════════════════════════ 12 値上がり前提
{
  ...
  s.addText("値上がりの織り込みは、10%確保のための前提条件です。", {
    x: M + 0.30, y: 5.56, w: 6.20, h: 0.26, margin: 0,
    fontFace: SANS, fontSize: 11.5, bold: true, color: GOLD_L,
  });
  s.addText("在庫回転12ヶ月で年率10%を確保するには、ワイン価格の年" + pct(D.need_rate_10, 2) + "以上の上昇が必要となる。主線の" + pct(D.appreciation, 0) + "に対する余裕は" + ((D.appreciation - D.need_rate_10) * 100).toFixed(1) + "ポイント。価格が横ばい（0%）にとどまると主線でも" + pct(RF(12).yld) + "となり10%を下回る。上乗せではなく前提条件である点にご留意いただきたい。", {
...
// ═══════════════════════════════════════════ 13 下振れ
{
  ...
  s.addText("値上がりを織り込めない場合、在庫回転を6ヶ月まで詰めない限り10%に届かない。", {
  ...
  s.addText("値引きと価格横ばいが重なると、どの回転期間でも10%に届かない。値引き販売を禁じることが最優先の防衛線となる。", {
  ...
  s.addText("「回転を上げるために値引く」は割に合わない。", {
    x: M + 0.30, y: 4.98, w: CW - 0.60, h: 0.30, margin: 0,
    fontFace: SERIF, fontSize: 16, bold: true, color: IVORY,
  });
  s.addText("売値5%の値引きは単位粗利を約" + pct(1 - (D.scen["ネガティブ"].gross12 / D.scen["ニュートラル"].gross12), 0) + "毀損する。在庫回転12ヶ月・価格横ばいの前提で比べると、値引きなしの " + pct(RF(12).yld) + " に対し、値引きありでは " + pct(RN(12).yld) + " まで低下する。ティア配分によって在庫回転の悪化は吸収できるが、値引きによる粗利の毀損は吸収できない。価格を維持したまま販売チャネルを増やすことが本筋の改善手段であり、売れ残りはむしろ値上がりを取りにいく機会でもある。契約上、一定率を超える値引き販売にはご承認をいただく建て付けとする。", {
...
// ═══════════════════════════════════════════ 18 リスク
{
  ...
  const risks = [
    ...
    ["価格変動リスク", "Fine Wine市況の下落・停滞", "本設計の最大のリスク。主線は年" + pct(D.appreciation, 0) + "上昇を織り込むが、10%確保には年" + pct(D.need_rate_10, 2) + "以上が必要。会社資料の年率10%に対し保守的な前提。"],
    ...
    ["カウンターパーティリスク", "WineBankの信用・実行力", "現物は貴社名義で保有されるため、WineBankの債権者から隔離される。器を介さないぶん、v1より隔離は強固。"],
...
// ═══════════════════════════════════════════ 19 設計条件
{
  ...
  const left = [
    ...
    [td("販売"), td("委託販売。WineBankが受託者として自己の名で販売し、所有権は貴社から顧客へ直接移転", { align: "left" })],
  ...
  table(s, left, M, 1.78, hw, [1.55, 4.25], { rowH: 0.44, fontSize: 8.5 });
  ...
  table(s, right, M + hw + 0.30, 1.78, hw, [1.55, 4.25], { rowH: 0.44, fontSize: 8.5 });
...
// ═══════════════════════════════════════════ 21 まとめ
{
  ...
  const pts = [
    ["01", "現物をご自身で直接ご所有いただく",
     "SPCも匿名組合も用いない。ワインは貴社名義で保有され、WineBankの債権者から隔離される。器の維持費（年660万円）も組成の手間も発生しない。"],
  ...
  const checks = [
    ["最低10%は確保したい",
     ...
     "ただしワイン価格の年" + pct(D.need_rate_10, 2) + "以上の上昇が前提。横ばいにとどまると " + pct(RF(12).yld) + " となり10%を下回る。"],
    ["20%は取り過ぎ",
