...
#
#   2026年8月のご指示により確定した水準。9ヶ月60% / 12ヶ月50% / 15ヶ月50% /
#   18ヶ月40% / 24ヶ月35% / 24ヶ月超30%。6ヶ月以内は70%で据え置き。
#   12ヶ月と15ヶ月が同率のため、両者は「15ヶ月以内50%」の一段にまとめている。
#   主線（回転12ヶ月）はこの段に属し、投資家利回りは年率14.3%となる。
#
#   〜6ヶ月 70% ／ 〜9ヶ月 60% ／ 〜15ヶ月 50% ／ 〜18ヶ月 40%
#   ／ 〜24ヶ月 35% ／ 24ヶ月超 30%
def wb_share(turn):
    if turn <= 6:  return 0.70
    if turn <= 9:  return 0.60
    if turn <= 15: return 0.50
    if turn <= 18: return 0.40
    if turn <= 24: return 0.35
    return 0.30
...
def tier_label(turn):
    ...
    if turn <= 15: return "15ヶ月以内"
    if turn <= 18: return "18ヶ月以内"
    if turn <= 24: return "24ヶ月以内"
    return "24ヶ月超"
...
def report():
    ...
    print(f"  配分       在庫回転期間に応じたティア"
          f"（6ヶ月70% / 9ヶ月60% / 15ヶ月以内50% / 18ヶ月40% / 24ヶ月35% / 24ヶ月超30%）")
