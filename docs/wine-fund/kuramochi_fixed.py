...
2026年8月21日の倉持氏からの提案。v2（利益配分型）から再度の全面変更。
  ...
  v3：**保有期間だけで投資家の取り分を固定する。**
      ...
      簿価に対して +10%（12ヶ月以内） / +20%（13〜24ヶ月） / +20%（24ヶ月超＝残存）。
      WineBankがそれ以上で売れた部分はWineBankの収益となる。

      本線の10% / 20% は倉持氏 8/21 18:21 の提示（1年目+10%・2年目+20%）。
      24ヶ月超の残存引き取りは当初30%としていたが、8/23 に倉持氏より買取条項が
      厳しいとのご指摘があり **+20%もしくは要協議、または共同で市場売却** に緩和した。
      対比として、8/21 19:37 にご提示いただいた 7.5% / 12.5% / 30% 案も併記する。
...
#
#   本線：12ヶ月以内 +10% ／ 13〜24ヶ月 +20% ／ 24ヶ月超 +20%
#         24ヶ月超は契約期間満了時に残存した在庫をWineBankが引き取る際の率であり、
#         「20%もしくは要協議、または共同で市場売却」とする（8/23 に30%から緩和）。
#   対比：8/21 19:37 にご提示いただいた 7.5% / 12.5% / 30% 案
#
#   売却はすべて24ヶ月以内に成立する前提で置いているため、24ヶ月超の率を変えても
#   投資家IRRの数値は動かない。影響するのは満了時の残存引き取り額のみ。
TIERS_MAIN = ((12, 0.10), (24, 0.20), (999, 0.20))
TIERS_ALT  = ((6, 0.075), (12, 0.125), (24, 0.30))

RESIDUAL_RATE = 0.20      # 24ヶ月時点で残存した在庫の引き取り率（もしくは要協議）
...
if __name__ == "__main__":
    ...
    print("=" * W)
    print(f"  投資家の取り分   12ヶ月以内 +10% ／ 13〜24ヶ月 +20% ／ 24ヶ月超（残存）+{RESIDUAL_RATE:.0%}")
...
def export_json(path="kuramochi_fixed.json"):
    ...
    data = dict(
        ...
        residual_main=residual(RESIDUAL_RATE),
        residual_rate=RESIDUAL_RATE,
        residual_cost=[dict(res=r, book=CAPITAL * r,
                            pay=CAPITAL * r * (1 + RESIDUAL_RATE))
