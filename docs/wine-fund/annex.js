...
children.push(
  ...
  p("あわせて、営業者が自己の計算において保有するワイン在庫と本匿名組合の在庫との間の配分の公平性についても、取扱いを明文化している。配分は在庫数量の比率によるプロラタを原則とし、プロラタにより決し得ない部分について本匿名組合の在庫を優先する構成としている。"),
...
children.push(article(3, "売却の配分"));
children.push(item(1, "営業者は、同一の対象銘柄について本件在庫および自己勘定在庫の双方を保有する場合、当該対象銘柄の売却を、売却時点における両者の在庫数量の比率に応じて配分するものとする。"));
children.push(item(2, "個々の売却は数量の分割になじまないことから、営業者は、対象銘柄ごとの累計売却数量の配分比率が前項の比率に一致するよう第8条に定める配分台帳により管理し、当該比率からの乖離が大きい側の在庫から出庫するものとする。"));
children.push(item(3, "前項による乖離が同等である場合、ロット単位での売却その他前項の方法により配分を決し得ない場合、または購入者の指定その他の事由により配分の判断を要する場合は、本件在庫を優先して売却するものとする。"));
children.push(item(4, "四半期の末日における累計配分比率と第1項の比率との乖離が●パーセントを超えた場合、営業者は、翌四半期においてこれを是正するものとする。"));
children.push(item(5, "購入者が自己勘定在庫を特定して指名した場合その他やむを得ない事由により前各項によらず売却を行った場合、営業者は、当該売却の日時、数量、単価および事由を記録し、配分台帳に記載するものとする。"));

children.push(article(4, "組入銘柄の選定"));
children.push(item(1, "営業者は、本件在庫に組み入れる対象銘柄の選定にあたり、流通量が多く売却までの期間が短いと見込まれる銘柄を優先するものとする。"));
children.push(item(2, "営業者は、本件在庫に組み入れる対象銘柄と自己勘定在庫に係る対象銘柄とが可能な限り重複しないよう努めるものとする。営業者は、組成時において両者の重複状況を調査し、その結果を匿名組合員に報告する。"));
children.push(item(3, "本匿名組合の在庫回転期間に係る運用目標（12か月）の達成は、主として本条に定める組入銘柄の選定によることを、両当事者は確認する。"));
...
children.push(item(2, "配分台帳には、対象銘柄ごとの累計売却数量の配分比率および第3条第1項の比率からの乖離を記載するものとする。"));
children.push(item(3, "営業者は、四半期ごとに配分台帳を匿名組合員に開示するものとする。"));

children.push(article(9, "出庫指示の自動割当"));
children.push(item(1, "営業者は、本件在庫および自己勘定在庫のいずれから出庫するかの決定を、第3条に定める規則に基づき情報システムにより自動的に行うものとし、担当者の裁量によらない運用とする。"));
...
[
  ...
  "営業者またはその関係会社との間の取引であって別紙3第4条に定めるもの",
].forEach((t, i) => children.push(sub(i + 1, t)));
...
children.push(item(4, "第1項の加算は、営業者が酒類販売業免許に基づく売買の当事者として在庫リスク、与信および入出庫事務を負担することの対価であり、次条に定める成功報酬とは別個のものとする。"));

children.push(article(3, "営業者の報酬"));
children.push(item(1, "営業者が本匿名組合から受領する報酬は、次項に定める成功報酬に限るものとし、営業者は、出資金額または純資産額に比例する期中の管理報酬を受領しない。"));
children.push(item(2, "成功報酬の額は、本匿名組合の税引前利益のうち匿名組合員の出資金額が本匿名組合の運用総額に占める割合に応じて匿名組合員に帰属する金額（以下「帰属利益」という。）の100分の50に相当する金額とする。"));
children.push(item(3, "匿名組合員に対する分配額は、帰属利益から前項の成功報酬を控除した金額とする。"));
children.push(item(4, "本匿名組合が損失を計上した事業年度については、成功報酬は発生しない。過年度の損失が繰り越されている場合、当該損失を回復した後の金額を基準として成功報酬を算定するものとする。"));
children.push(item(5, "本件ワインの販売、保管の手配その他の運営に係る営業者側の人件費および販売関連費用のうち、別紙2および本契約において本匿名組合の負担と明記されたものを除く費用は、営業者が前項までの報酬の中から負担するものとし、本匿名組合に請求しない。"));
children.push(item(6, "営業者は、各事業年度の成功報酬の算定根拠を、別紙2第2条に定める年次報告において開示するものとする。"));

children.push(article(4, "関連当事者取引の事前承認"));
...
const todo = [
  ...
  ["別紙3 第4条", "事前承認を要する関連当事者取引の金額基準"],
  ["別紙3 第5条", "取引記録の保存期間"],
  ["別紙1 第3条(4)", "累計配分比率の乖離に係る許容幅"],
