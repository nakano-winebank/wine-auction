...
// ═══════════════════════════════════════════════════════════ 02 サマリー
{
  ...
  stat(s, M + (tw + 0.30) * 1, 1.82, tw, 1.62, "5.0", "億円", "前提となる年間販売", "現状の販売実力5〜7億円の中位に置いた保守的な主線");
  stat(s, M + (tw + 0.30) * 2, 1.82, tw, 1.62, "29.1", "%", "エクイティ定常年間利回り", "年間販売5.0億・在庫回転18.5ヶ月・投資家取分60%");
  stat(s, M + (tw + 0.30) * 3, 1.82, tw, 1.62, "24.4", "%", "年平均利回り（5年通算）", "累計分配額÷出資額÷5年／税引前",
    { vs: 30, fill: CARD2, line: GOLD_D });

  const pts = [
    ["現状の販売実力のなかで20%を達成する設計にしている。",
     "WineBankの年間販売実力は5〜7億円。その中位5.0億円を主線に置き、在庫回転18.5ヶ月でエクイティ利回り29.1%。年間販売が3.2億円を下回らない限り20%を維持する。"],
    ["収益は「流通価格差」と「ワインの値上がり」の二階建て。",
     "同一商品の流通段階間の価格差（フロー）に加え、保有期間中の価格上昇年6%（ストック）が乗る。値上がり分は保有期間によらず利回りを底上げする。"],
    ["デット2億の調達コストはWineBankが負担する。",
     "その見返りにWineBankはエクイティ2億相当としてカウントされ、SPCの税前利益は出資比率で按分（投資家60：WineBank40）。投資家はレバレッジの効果のみを受け取る。"],
...
// ═══════════════════════════════════════════════════════════ 03 資本構成
{
  ...
  const rows = [
    [th("項目"), th("金額"), th("内容")],
    [td("年間税前利益（SPC）"), td("1.46億円", { bold: true }), td("年間販売5.0億円・在庫回転18.5ヶ月の主線ケース", { align: "left" })],
    [td("├ 投資家 60%"), td("87百万円", { bold: true, color: GOLD_L }), td("エクイティ3億円に対し 定常年間利回り 29.1%", { align: "left", color: GOLD_L, bold: true })],
    [td("└ WineBank 40%"), td("58百万円"), td("うちデット2億円の金利4%（8百万円）を負担 → 手取り50百万円", { align: "left" })],
  ...
  s.addText("従来の折半（50:50）であればエクイティ利回りは24.3%。出資比率按分（60:40）とし、デットの調達コストをWineBank側で負担することで29.1%となる。投資家はレバレッジによる収益拡大を受け取る一方、金利変動リスクと保証責任はWineBankが引き受ける。なお同一投資家がデット2億円とエクイティ3億円の双方を拠出する場合、5億円全体に対する合成利回りは19.1%となるため、トランシェを分けて調達することを前提とする。", {
...
// ═══════════════════════════════════════════════════════════ 04 販売力とファンド規模
{
  const s = chrome(base(), "SALES CAPACITY", "販売実績とリターンの関係",
    "本ファンドのリターンは、WineBankが年間いくら売れるかで決まる。現状の販売実力を起点に設計している。");

  const rows = [
    [th("年間販売実績"), th("在庫回転期間"), th("年間税前利益"), th("投資家取分（60%）"), th("エクイティ定常年間利回り"), th("年平均利回り 5年通算"), th("WineBank取分")],
    [td("4.0億円"), td("23.8ヶ月"), td("1.20億円"), td("72百万円"), td("23.9%", { bold: true }), td("18.8%"), td("40百万円")],
    [td("4.5億円"), td("20.8ヶ月"), td("1.33億円"), td("80百万円"), td("26.5%", { bold: true }), td("18.3%"), td("45百万円")],
    [td("5.0億円（主線）", { bold: true, color: GOLD_L }), td("18.5ヶ月"), td("1.46億円"), td("87百万円"), td("29.1%", { bold: true, color: GOLD_L }), td("24.4%", { color: GOLD_L }), td("50百万円")],
    [td("6.0億円"), td("15.2ヶ月"), td("1.71億円"), td("103百万円"), td("34.3%", { bold: true }), td("25.9%"), td("61百万円")],
    [td("7.0億円"), td("12.9ヶ月"), td("1.98億円"), td("119百万円"), td("39.5%", { bold: true }), td("33.7%"), td("71百万円")],
  ];
  table(s, rows, M, 1.80, CW, [1.90, 1.55, 1.60, 1.80, 2.28, 1.60, 1.16], { rowH: 0.40 });

  const cards3 = [
    ["現状の販売実力", "5〜7億円", "楽天・Yahoo・寺田Wine MarketでのEC、酒販店卸、グループ飲食。頑張れば伸ばせるが10億円は難しい水準として、中位の5.0億円を主線に置いた。", false],
    ["20%を割り込むライン", "年間販売 3.2億円", "現状の販売実力に対して4割の水準まで落ち込まない限り、貴社基準の20%は維持される。下限側に相応の余裕がある。", true],
    ["販売力を伸ばす打ち手", "海外・オークション", "輸出入ライセンスを活かした海外販売、2026年9月開始のWineBank Auction、グループ飲食（アピシウス／ティエリーマルクス）。これらは出資金ではなくWineBank側の資本で手当てする。", false],
  ];
  const cw3 = (CW - 0.28 * 2) / 3;
  cards3.forEach((c, i) => {
    const x = M + (cw3 + 0.28) * i;
    card(s, x, 4.20, cw3, 1.96, { fill: c[3] ? CARD2 : CARD, line: c[3] ? GOLD_D : LINE });
    s.addText(c[0], { x: x + 0.26, y: 4.38, w: cw3 - 0.52, h: 0.26, margin: 0, fontFace: SANS, fontSize: 11, bold: true, color: c[3] ? GOLD_L : IVORY });
    s.addText(c[1], { x: x + 0.26, y: 4.68, w: cw3 - 0.52, h: 0.40, margin: 0, valign: "middle", fontFace: SANS, fontSize: 18, bold: true, color: GOLD_L });
    s.addText(c[2], { x: x + 0.26, y: 5.14, w: cw3 - 0.52, h: 0.90, margin: 0, fontFace: SANS, fontSize: 9, color: MUTED, lineSpacingMultiple: 1.24 });
  });

  s.addText("※ 在庫回転期間は年間販売実績から逆算した結果であり、前提として置いた数値ではない。「回転12ヶ月を目指す」という計画ではなく、「現に売れている金額でいくらになるか」を示している。年平均利回り（5年通算）が単調でないのは、満期までに何回転が収まるかという期間の刻みによる。", {
    x: M, y: 6.26, w: CW, h: 0.40, margin: 0, fontFace: SANS, fontSize: 8.5, color: DIM, lineSpacingMultiple: 1.2 });

  s.addNotes("販売実績から逆算する構成。20%割れは年間販売3.2億円を下回った場合のみ。");
}
...
// ═══════════════════════════════════════════════════════════ 09 ベースケース表
{
  const s = chrome(base(), "BASE CASE ｜ 主線", "ニュートラル：在庫回転期間別シミュレーション",
    "SPC取得原価50.5・ワイン価格上昇 年6%／総額5億円（デット2億＋エクイティ3億）・運用期間5年・稼働率95%");

  const rows = [
    [th("在庫回転期間"), th("年回転数"), th("年間売上"), th("年間粗利"), th("SPC費用"), th("年間税前利益"), th("投資家取分（60%）"), th("エクイティ定常年間利回り"), th("5年累計分配"), th("年平均利回り 5年通算")],
    [td("12ヶ月"), td("1.00回"), td("7.48億"), td("2.73億"), td("63百万"), td("2.10億"), td("126百万"), td("42.0%", { bold: true }), td("502百万"), td("33.5%", { bold: true, color: GOLD_L })],
    [td("15ヶ月"), td("0.80回"), td("6.07億"), td("2.27億"), td("54百万"), td("1.73億"), td("104百万"), td("34.7%", { bold: true }), td("387百万"), td("25.8%", { bold: true, color: GOLD_L })],
    [td("18.5ヶ月（主線）", { bold: true, color: GOLD_L }), td("0.65回"), td("5.01億"), td("1.92億"), td("47百万"), td("1.46億"), td("87百万"), td("29.1%", { bold: true, color: GOLD_L }), td("368百万"), td("24.5%", { bold: true, color: GOLD_L })],
    [td("21ヶ月"), td("0.57回"), td("4.46億"), td("1.75億"), td("43百万"), td("1.32億"), td("79百万"), td("26.3%", { bold: true }), td("275百万"), td("18.3%", { bold: true })],
    [td("24ヶ月"), td("0.50回"), td("3.96億"), td("1.59億"), td("40百万"), td("1.19億"), td("71百万"), td("23.8%", { bold: true }), td("283百万"), td("18.9%", { bold: true })],
  ];
  table(s, rows, M, 1.82, CW, [1.72, 0.86, 0.98, 0.98, 0.92, 1.16, 1.40, 1.55, 1.02, 1.30], { rowH: 0.40 });
  ...
  const read = [
    "定常年間利回り＝運用が軌道に乗った後の年間実力値。エクイティ3億円に対する比率",
    "年平均利回り＝5年間の累計分配額÷エクイティ3億円÷5年。仕入展開6ヶ月と満期前の清算期間を織り込んだ通算値",
    "主線は年間販売5.0億円から逆算した在庫回転18.5ヶ月。回転を前提として置いてはいない",
  ...
  s.addText("エクイティ定常年間利回り（税引前）", {
    x: M + 6.63, y: 4.54, w: CW - 6.95, h: 0.24, margin: 0,
    fontFace: SANS, fontSize: 10.5, color: IVORY });
  s.addText([
    { text: "23.8 – 42.0", options: { fontFace: LATIN, fontSize: 30, bold: true, color: GOLD_L } },
    { text: " %", options: { fontFace: SANS, fontSize: 15, bold: true, color: GOLD_L } },
  ], { x: M + 6.63, y: 4.78, w: CW - 6.95, h: 0.50, margin: 0, valign: "middle" });
  s.addText("主線は年間販売5.0億円に対応する在庫回転18.5ヶ月で29.1%。回転24ヶ月（年間販売3.96億円）まで悪化しても23.8%を確保する。20%を割り込むのは年間販売が3.2億円を下回った場合。", {
...
// ═══════════════════════════════════════════════════════════ 10 回転とリターン
{
  const s = chrome(base(), "BASE CASE ｜ 主線", "年間販売実績と投資家リターンの関係",
    "横軸は在庫回転期間ではなく年間販売実績。WineBankがいくら売れるかがそのままリターンになる。");

  const cats = ["4.0億", "5.0億", "6.0億", "7.0億"];
  const specs = [
    ["エクイティ定常年間利回り（％）", [23.9, 29.1, 34.3, 39.5], 45, GOLD],
    ["年平均利回り（5年通算・％）", [18.8, 24.4, 25.9, 33.7], 39, GOLD_D],
  ];
  ...
  const notes = [
    ["販売実績が主たる決定要因", "年間販売が1億円増えるごとにエクイティ利回りは約5ポイント改善する。値引きや粗利率の微調整より、売る量を増やす打ち手のほうが効く。"],
    ["ただし値上がりが下支えする", "販売が4.0億円まで落ちても23.9%を確保できる。保有が長いぶん売値が上がるためで、値上がりを織り込まない場合とは約8ポイントの差がつく（次頁）。"],
  ];
  notes.forEach((n, i) => {
...
// ═══════════════════════════════════════════════════════════ 11 値上がりの織り込み
{
  ...
  // 分解テーブル
  const rows = [
    [th("在庫回転期間"), th("売値の上昇倍率"), th("① 価格差による利回り（上昇0%）"), th("② 値上がりによる上乗せ"), th("エクイティ利回り（①＋②）")],
    [td("12ヶ月"), td("×1.060"), td("34.1%"), td("＋7.9pt", { color: GOLD_L }), td("42.0%", { bold: true, color: GOLD_L })],
    [td("15ヶ月"), td("×1.076"), td("26.7%"), td("＋8.0pt", { color: GOLD_L }), td("34.7%", { bold: true, color: GOLD_L })],
    [td("18.5ヶ月（主線）", { bold: true, color: GOLD_L }), td("×1.094"), td("21.1%"), td("＋8.0pt", { color: GOLD_L }), td("29.1%", { bold: true, color: GOLD_L })],
    [td("21ヶ月"), td("×1.107"), td("18.2%"), td("＋8.1pt", { color: GOLD_L }), td("26.3%", { bold: true, color: GOLD_L })],
    [td("24ヶ月"), td("×1.124"), td("15.6%"), td("＋8.2pt", { color: GOLD_L }), td("23.8%", { bold: true, color: GOLD_L })],
  ];
  ...
  const rates = [["0%（横ばい）", "21.1%"], ["6%（主線）", "29.1%"], ["10%（会社資料水準）", "34.6%"]];
  rates.forEach((r, i) => {
...
// ═══════════════════════════════════════════════════════════ 12 上振れ・下振れ
{
  const s = chrome(base(), "UPSIDE / DOWNSIDE ｜ 副次", "上振れ余地と下振れリスク",
    "いずれも年間販売実績を横軸に置いている。回転期間は販売金額から逆算した結果。");
  ...
  table(s, [
    [th("年間販売"), th("在庫回転"), th("年間税前"), th("投資家取分"), th("エクイティ利回り")],
    [td("4.0億"), td("33.1ヶ月"), td("1.67億"), td("100百万"), td("33.4%", { bold: true, color: GOLD_L })],
    [td("5.0億"), td("25.6ヶ月"), td("2.05億"), td("123百万"), td("40.9%", { bold: true, color: GOLD_L })],
    [td("6.0億"), td("20.8ヶ月"), td("2.43億"), td("146百万"), td("48.5%")],
    [td("7.0億"), td("17.6ヶ月"), td("2.81億"), td("168百万"), td("56.1%")],
  ], M + 0.28, 2.32, 5.23, [0.98, 1.10, 1.05, 1.05, 1.05], { rowH: 0.325, fontSize: 9.5 });
  s.addText("インポーター直仕入の比率を引き上げ、販売をネット最安値帯に寄せることで到達。単価が上がるぶん、同じ販売金額でも回転はゆるやかになる。年間調達枠の確保が前提条件。", {
  ...
  table(s, [
    [th("年間販売"), th("在庫回転"), th("年間税前"), th("投資家取分"), th("エクイティ利回り")],
    [td("4.0億"), td("20.1ヶ月"), td("0.76億"), td("46百万"), td("15.2%", { color: RED })],
    [td("4.5億"), td("17.9ヶ月"), td("0.88億"), td("53百万"), td("17.5%")],
    [td("5.0億"), td("16.1ヶ月"), td("0.99億"), td("59百万"), td("19.8%", { bold: true })],
    [td("6.0億"), td("13.4ヶ月"), td("1.21億"), td("73百万"), td("24.3%", { bold: true })],
  ], nx + 0.28, 2.32, 5.23, [0.98, 1.10, 1.05, 1.05, 1.05], { rowH: 0.325, fontSize: 9.5 });
  s.addText("値引き（▲4.7pt）と価格横ばい（▲8.0pt）が同時に起きた複合ケース。この場合の20%の分岐点は年間販売5.05億円となり、現状の販売実力の下限に重なる。", {
  ...
  s.addText("「回転を上げるために値引く」は割に合わない。売値5%の値引きは単位粗利を15%毀損し、失うエクイティ利回り4.7ポイントを取り戻すには年間販売を5.0億円から約6.0億円へ引き上げる必要がある。価格を維持したまま販売チャネルを増やすことが本筋の改善手段であり、むしろ売れ残りは値上がりを取りにいく機会でもある。", {
...
// ═══════════════════════════════════════════════════════════ 12 3シナリオ比較
{
  const s = chrome(base(), "COMPARISON", "3シナリオ比較：エクイティ定常年間利回り",
    "エクイティ3億円に対する年間実力値。投資家取分（SPC税前利益の60%）ベース。");

  const cats = ["年間販売 4.0億", "年間販売 5.0億", "年間販売 6.0億"];
  card(s, M, 1.80, CW, 2.98);
  s.addChart(pres.ChartType.bar, [
    { name: "ポジティブ", labels: cats, values: [33.4, 40.9, 48.5] },
    { name: "ニュートラル（主線）", labels: cats, values: [23.9, 29.1, 34.3] },
    { name: "ネガティブ", labels: cats, values: [15.2, 19.8, 24.3] },
  ], Object.assign({}, CHART_BASE, {
    x: M + 0.14, y: 1.92, w: CW - 0.28, h: 2.74,
    barDir: "col", barGapWidthPct: 45,
    title: "年間販売実績別 エクイティ定常年間利回り（％）",
  ...
  const cols = [
    ["主線の提示レンジ", "現状の販売実力5〜7億円に対応するエクイティ利回りは29.1〜39.5%。中位の5.0億円を主線として提示する。", true],
    ["上振れの条件", "調達をインポーター直に寄せ、販売をネット最安値帯に寄せれば、同じ販売金額で利回りは約1.4倍になる。", false],
    ["下振れの限界", "値引き5%と価格横ばいが重なった場合、20%の分岐点は年間販売5.05億円。ここが最も厳しい防衛線となる。", false],
  ];
...
// ═══════════════════════════════════════════════════════════ 13 感応度
{
  const s = chrome(base(), "SENSITIVITY", "感応度：何が崩れると利回りがどう動くか",
    "主線（年間販売5.0億円・在庫回転18.5ヶ月・エクイティ利回り29.1%）を基準とした、各変数の単独変動による影響。");

  const rows = [
    [th("変動要因"), th("変動幅"), th("エクイティ利回りへの影響"), th("深刻度"), th("対応方針")],
    [td("ワイン価格上昇率"), td("年6% → 0%（横ばい）"), td("29.1% → 21.1%（▲8.0pt）", { color: RED, bold: true }), td("最大", { color: RED, bold: true }), td("回転期間を短縮し、価格差収益の比重を高める", { align: "left" })],
    [td("年間販売実績"), td("5.0億円 → 4.0億円"), td("29.1% → 23.9%（▲5.2pt）", { color: RED, bold: true }), td("大", { color: RED }), td("海外販売・オークションによる販路の多重化", { align: "left" })],
    [td("売却価格"), td("売値 ▲5%（70/80→66.5/76）"), td("29.1% → 24.4%（▲4.7pt）", { color: RED }), td("大"), td("値引き販売を禁じ、価格維持を運用ルール化", { align: "left" })],
    [td("年間販売実績"), td("5.0億円 → 6.0億円"), td("29.1% → 34.3%（＋5.2pt）", { color: GOLD_L }), td("上振れ", { color: GOLD_L }), td("販売力の増強がそのままリターンに直結する", { align: "left" })],
    [td("ワイン価格上昇率"), td("年6% → 10%（会社資料水準）"), td("29.1% → 34.6%（＋5.5pt）", { color: GOLD_L }), td("上振れ", { color: GOLD_L }), td("Liv-ex連動銘柄の構成比を高める", { align: "left" })],
    [td("調達構成"), td("定価比 50 → 45"), td("29.1% → 39.3%（＋10.2pt）", { color: GOLD_L, bold: true }), td("上振れ", { color: GOLD_L, bold: true }), td("インポーター直取引比率の引き上げが最大の改善策", { align: "left" })],
  ];
  table(s, rows, M, 1.80, CW, [1.96, 2.42, 2.58, 0.96, 3.97], { rowH: 0.42 });

  card(s, M, 5.14, CW, 1.10, { fill: CARD2, line: GOLD_D });
  s.addText("結論：エクイティ利回りが20%を割り込むのは、年間販売が3.2億円を下回った場合のみ（ニュートラル前提）。値引き5%と価格横ばいが同時に起きる複合的な下振れ（ネガティブ）では、年間販売5.05億円が20%の分岐点となる。なお、インポーターの仕入価格が上昇する局面では希望小売価格も連動して上昇するため、定価比の構造（仕入50・売却70／80）は維持され、利回りへの影響は生じない。", {
...
// ═══════════════════════════════════════════════════════════ 在庫配分の公平性
{
  const s = chrome(base(), "FAIR ALLOCATION", "WineBank在庫との公平性をどう担保するか",
    "調達・保管・販売のすべてをWineBankが担うため、自己勘定在庫との配分に構造的な利益相反が生じる。");

  const cw = (CW - 0.28 * 2) / 3;
  const layers = [
    ["01", "銘柄排他を原則とする", "SKU（銘柄×ヴィンテージ×フォーマット）単位で、SPC組入銘柄とWineBank自己勘定在庫を重複させない。買い手が銘柄を指名した時点で出所が一意に決まるため、配分の判断自体が発生しない。Liv-ex掲載の流動性上位銘柄はSPCに優先配分する。", false],
    ["02", "重複銘柄はSPC優先売却", "大量割当を按分取得した場合など重複が残る銘柄については、当該SKUのSPC在庫が完売するまでWineBankは自己勘定を売却しない。裁量の余地がなく、突合も容易である。", false],
    ["03", "価格同一ルールと補填条項", "同一SKU・同一四半期の実現単価は、SPCがWineBank自己勘定を下回らないものとする。下回った場合は差額×SPC数量をWineBankがSPCへ現金で補填する。運用が崩れても投資家が金額で守られる仕組みとする。", true],
  ];
  layers.forEach((l, i) => {
    const x = M + (cw + 0.28) * i;
    card(s, x, 1.80, cw, 2.32, { fill: l[3] ? CARD2 : CARD, line: l[3] ? GOLD_D : LINE });
    badge(s, x + 0.26, 2.02, 0.34, l[0]);
    s.addText(l[1], { x: x + 0.70, y: 2.04, w: cw - 0.96, h: 0.30, margin: 0, valign: "middle", fontFace: SANS, fontSize: 11.5, bold: true, color: l[3] ? GOLD_L : IVORY });
    s.addText(l[2], { x: x + 0.26, y: 2.54, w: cw - 0.52, h: 1.46, margin: 0, fontFace: SANS, fontSize: 9.5, color: MUTED, lineSpacingMultiple: 1.26 });
  });

  const rows = [
    [th("担保の手段"), th("内容")],
    [td("取得時の按分"), td("同一SKUを複数ロット取得する場合は、SPCとWineBank自己勘定の在庫比率に応じて按分して割り当てる", { align: "left" })],
    [td("配分台帳"), td("SKU別に、期首在庫・取得・売却・期末在庫をSPCとWineBank自己勘定の両建てで記録し、四半期ごとに投資家へ開示する", { align: "left" })],
  ...
  table(s, rows, M, 4.22, CW, [2.60, 9.29], { rowH: 0.335 });

  s.addText("※ 組成前にSPC組入予定銘柄とWineBank自己勘定在庫の重複度を精査し、重複が残る銘柄を特定したうえで02・03の適用範囲を確定する。実務上は「01 銘柄排他」でおおむね解消できる見込み。", {
    x: M, y: 6.42, w: CW, h: 0.34, margin: 0, fontFace: SANS, fontSize: 8.5, color: DIM, lineSpacingMultiple: 1.2 });
  s.addNotes("SKU排他が主。重複分はSPC優先売却＋価格同一＋補填条項で担保。");
}
...
// ═══════════════════════════════════════════════════════════ 15 条件
{
  ...
  const right = [
    ...
    [td("販売チャネル"), td("酒販店卸（B2B）と自社EC・オークション（B2C）を半々。手取りの高いチャネルを優先", { align: "left" })],
    ...
    [td("想定リターン"), td("エクイティ定常年間利回り29.1%（年間販売5.0億円）。5年通算の年平均利回りは24.4%", { align: "left", color: GOLD_L, bold: true })],
    [td("報告"), td("四半期：在庫明細・簿価・売上・試算表／年次：決算", { align: "left" })],
    [td("外部検証"), td("AUP（合意された手続）を年次で実施。配分ルールの遵守状況を手続の対象に含める", { align: "left" })],
    [td("在庫配分"), td("SKU排他を原則とし、重複銘柄はSPC優先売却。契約別紙として明文化する", { align: "left" })],
  ];
  ...
  s.addText("半期解約を前提とすると常時1〜3割を現金または即時換金可能な在庫で保有する必要があり、稼働率が下がって利回りも下がる（稼働率95%→85%でエクイティ利回りは29.1%→26.0%）。当初3年ロックアップ・4年目以降は年度ごとの解約という設計により、現金保有を5%に抑え、総額の95%をワインに充当できる。", {
...
// ═══════════════════════════════════════════════════════════ 17 まとめ
{
  ...
  const merits = [
    ...
    ["収益が二階建てで、片方が崩れても成立する",
     "流通価格差（回転に対するリターン）とワインの値上がり（在庫に対するリターン）。価格が横ばいでもエクイティ利回り21.1%、売れ行きが鈍っても値上がりが年8.0ポイントを底上げする。"],
    ["他社が再現できない調達力",
    ...
    ["運営者と利害が完全に一致している",
     "期中報酬も成功報酬も取らない。デット2億円の調達コストをWineBankが負担し、損益は出資比率で按分する。投資家が儲からなければWineBankも儲からない。"],
  ];
  let my = 1.72;
  merits.forEach((m, i) => {
  ...
  const checks = [
    ["想定利回り20%（実績）に対して",
     "現状の販売実力5〜7億円のうち中位5.0億円を主線に置き、エクイティ利回り29.1%。20%を割り込むのは年間販売が3.2億円を下回ったとき。",
     "年間販売3.2億円が分岐点。現状実力の6割の水準まで落ちない限り貴社基準を維持する。"],
    ["解約条件に対して",
     "運用期間5年・当初3年ロックアップ・4年目以降は年度ごとの解約。",
     "この設計により現金保有を5%に抑え、総額の95%をワインに充当できる。半期解約を前提とすると稼働率が下がり29.1%→26.0%に低下する。"],
    ["リスク分散に対して",
