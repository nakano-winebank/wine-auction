...
// ═══════════════════════════════════════════════════════════ 02 サマリー
{
  ...
  stat(s, M + (tw + 0.30) * 3, 1.82, tw, 1.62, "11.0–16.9", "%", "投資家IRR（3年）", "回転12ヶ月〜9ヶ月／税引前",
    { vs: 24, fill: CARD2, line: GOLD_D });

  const pts = [
    ["リターンを決めるのは粗利率ではなく在庫回転期間。", "粗利率は前提として固定されるため、投資家リターンは「在庫を年に何回まわせるか」でほぼ決まる。回転9ヶ月でIRR16.9%、12ヶ月で11.0%。"],
  ...
  s.addNotes("骨子。IRRの主線は回転9〜12ヶ月で11.0〜16.9%。");
}
...
// ═══════════════════════════════════════════════════════════ 03 改訂差分
{
  ...
  const rows = [
    [th("在庫回転期間"), th("定常利回り（8/5版）"), th("定常利回り（今回版）"), th("差"),
     th("投資家IRR 3年（8/5版）"), th("投資家IRR 3年（今回版）"), th("差")],
    [td("9ヶ月"), td("24.3%"), td("23.1%"), td("▲1.2pt", { color: RED }), td("17.9%"), td("16.9%"), td("▲1.0pt", { color: RED })],
    [td("12ヶ月（主線）", { bold: true, color: GOLD_L }), td("17.8%"), td("16.9%", { bold: true, color: GOLD_L }), td("▲1.0pt", { color: RED }), td("11.7%"), td("11.0%", { bold: true, color: GOLD_L }), td("▲0.7pt", { color: RED })],
    [td("15ヶ月"), td("14.0%"), td("13.1%"), td("▲0.8pt", { color: RED }), td("11.0%"), td("10.3%"), td("▲0.7pt", { color: RED })],
  ...
  s.addText("※ 8/5版の列は、今回と同一の月次モデルに旧前提（譲渡マージンなし・人件費なし）を入れて再計算したもの。8/5版の公表IRR（12ヶ月＝11.4%）との0.3ポイント差は、月次キャッシュフローの精緻化による計算差であり、前提の変更によるものではない。", {
    x: M, y: 5.90, w: CW, h: 0.44, margin: 0,
    fontFace: SANS, fontSize: 8.5, color: DIM, lineSpacingMultiple: 1.2,
  });
  s.addNotes("改訂は2点。影響は定常利回りで▲1.0pt、IRRで▲0.7pt。粗利率は33.3%→32.7%となる。");
}
...
// ═══════════════════════════════════════════════════════════ 08 費用前提
{
  ...
  s.addText("上記はニュートラル・回転12ヶ月・稼働率85%における年間SPC費用（合計3,739万円）の内訳。その他の運用前提：稼働率85%（残15%は運転資金・解約準備として現金保有）／仕入展開6ヶ月／ローリング運用／利益はSPC内で複利再投資せず半期分配／税前利益を投資家50：WineBank50で折半／IRRは投資家の税引前ベース。", {
...
// ═══════════════════════════════════════════════════════════ 09 ベースケース表
{
  ...
  const rows = [
    [th("在庫回転期間"), th("年回転数"), th("年間売上"), th("年間粗利"), th("SPC費用"), th("年間税前利益"), th("投資家配当／年"), th("定常利回り"), th("IRR（3年）"), th("IRR（5年）")],
    [td("9ヶ月"), td("1.33回"), td("8.42億"), td("2.75億"), td("44百万"), td("2.31億"), td("116百万"), td("23.1%", { bold: true }), td("16.9%", { bold: true, color: GOLD_L }), td("20.0%")],
    [td("12ヶ月", { bold: true, color: GOLD_L }), td("1.00回"), td("6.31億"), td("2.06億"), td("37百万"), td("1.69億"), td("84百万"), td("16.9%", { bold: true }), td("11.0%", { bold: true, color: GOLD_L }), td("13.3%")],
    [td("15ヶ月"), td("0.80回"), td("5.05億"), td("1.65億"), td("33百万"), td("1.31億"), td("66百万"), td("13.1%", { bold: true }), td("10.3%", { bold: true, color: GOLD_L }), td("9.6%")],
    [td("18ヶ月"), td("0.67回"), td("4.21億"), td("1.37億"), td("31百万"), td("1.07億"), td("53百万"), td("10.7%", { bold: true }), td("5.1%", { bold: true, color: RED }), td("9.0%")],
  ];
  ...
  const read = [
    "定常利回り＝運用が軌道に乗った後の年間実力値",
    "IRRは仕入展開6ヶ月と満期前の清算期間を織り込んだ実績値",
...
// ═══════════════════════════════════════════════════════════ 10 回転とリターン（チャート）
{
  ...
  const specs = [
    ...
    ["投資家IRR（3年）", [16.9, 11.0, 10.3, 5.1], 22],
    ["投資家IRR（5年）", [20.0, 13.3, 9.6, 9.0], 24],
  ];
...
// ═══════════════════════════════════════════════════════════ 11 上振れ・下振れ
{
  ...
  table(s, [
    [th("回転期間"), th("年間売上"), th("年間税前"), th("定常利回り"), th("IRR（3年）")],
    [td("9ヶ月"), td("11.22億"), td("5.01億"), td("50.1%"), td("36.7%", { bold: true, color: GOLD_L })],
    [td("12ヶ月"), td("8.42億"), td("3.71億"), td("37.1%"), td("24.3%", { bold: true, color: GOLD_L })],
    [td("15ヶ月"), td("6.73億"), td("2.93億"), td("29.3%"), td("22.2%")],
    [td("18ヶ月"), td("5.61億"), td("2.41億"), td("24.1%"), td("11.8%")],
  ...
  table(s, [
    [th("回転期間"), th("年間売上"), th("年間税前"), th("定常利回り"), th("IRR（3年）")],
    [td("9ヶ月"), td("8.00億"), td("1.90億"), td("19.0%"), td("13.9%", { bold: true })],
    [td("12ヶ月"), td("6.00億"), td("1.38億"), td("13.8%"), td("9.0%", { bold: true })],
    [td("15ヶ月"), td("4.80億"), td("1.07億"), td("10.7%"), td("8.4%")],
...
// ═══════════════════════════════════════════════════════════ 12 3シナリオ比較
{
  const s = chrome(base(), "COMPARISON", "3シナリオ比較：投資家IRR（3年）",
    "現物譲渡1%・SPC人件費360万円を反映した後の投資家IRR。");

  const cats = ["9ヶ月", "12ヶ月", "15ヶ月", "18ヶ月"];
  card(s, M, 1.80, CW, 2.98);
  s.addChart(pres.ChartType.bar, [
    { name: "ポジティブ", labels: cats, values: [36.7, 24.3, 22.2, 11.8] },
    { name: "ニュートラル（主線）", labels: cats, values: [16.9, 11.0, 10.3, 5.1] },
    { name: "ネガティブ", labels: cats, values: [13.9, 9.0, 8.4, 4.1] },
  ], Object.assign({}, CHART_BASE, {
    x: M + 0.14, y: 1.92, w: CW - 0.28, h: 2.74,
    barDir: "col", barGapWidthPct: 45,
    title: "在庫回転期間別 投資家IRR（3年・％）",
  ...
  const cols = [
    ["主線の提示レンジ", "回転9〜12ヶ月でIRR11.0〜16.9%。保守的な前提を積み上げた数値であり、交渉の出発点として提示する。", true],
    ["上振れの条件", "調達をインポーター直に寄せ、販売を自社チャネルに寄せれば、同じ回転期間でIRRは2倍以上になる。", false],
    ["下振れの限界", "値引き5%を強いられても、回転9ヶ月を維持できればIRR13.9%を確保できる。回転の維持が防衛線。", false],
  ];
...
// ═══════════════════════════════════════════════════════════ 15 条件
{
  ...
  const rows = [
    ...
    [td("想定リターン"), td("投資家IRR 11.0〜16.9%（回転12〜9ヶ月）", { align: "left" }), td("ニュートラル前提。上振れ余地は前掲", { align: "left" })],
  ...
  s.addText("半期解約に応じるには常時1〜3割を現金または即時換金可能な在庫で保有する必要があり、その分だけ稼働率が下がる。稼働率85%→70%で定常利回りは16.9%→13.7%、IRR（3年）は11.0%→8.9%に低下する。解約条件はご要望に合わせて設計するが、利回りとの両立が難しい点を事前に共有したい。", {
