...
// ═══════════════════════════════════════════════════════════ 02 サマリー
{
  ...
  stat(s, M + (tw + 0.30) * 2, 1.82, tw, 1.62, "42.0", "%", "エクイティ定常年間利回り", "在庫回転12ヶ月・投資家取分60%・税引前");
  stat(s, M + (tw + 0.30) * 3, 1.82, tw, 1.62, "33.5", "%", "年平均利回り（5年通算）", "在庫回転12ヶ月／累計分配額÷出資額÷5年",
    { vs: 30, fill: CARD2, line: GOLD_D });

  const pts = [
    ["在庫回転12ヶ月で売り切る。SPC在庫を優先して販売する。",
     "回転12ヶ月には年間7.5億円の販売が必要となる。WineBank自己勘定在庫より先にSPC在庫を売る運用ルールとし、これを契約上の義務とする。最短9ヶ月、延びても15〜18ヶ月を想定レンジとする。"],
...
// ═══════════════════════════════════════════════════════════ 03 資本構成
{
  ...
  const rows = [
    ...
    [td("├ 投資家 60%"), td("126百万円", { bold: true, color: GOLD_L }), td("エクイティ3億円に対し 定常年間利回り 42.0%", { align: "left", color: GOLD_L, bold: true })],
    [td("└ WineBank 40%"), td("84百万円"), td("うちデット2億円の金利4%（8百万円）を負担 → 手取り76百万円", { align: "left" })],
  ...
  s.addText("投資家の出資比率（3億円÷5億円＝60%）と損益の配分比率（60%）が一致しているため、エクイティ利回りはSPCの資産利回りそのものとなる。レバレッジによる上振れはWineBankの取り分（無拠出で40%）とデットの金利負担で相殺されており、投資家が過度なレバレッジを負う構造にはなっていない。なお同一投資家がデット2億円とエクイティ3億円の双方を拠出する場合、5億円全体に対する合成利回りは27.2%となる。", {
...
// ═══════════════════════════════════════════════════════════ 04 販売計画
{
  const s = chrome(base(), "SALES PLAN", "在庫回転12ヶ月をどう実現するか",
    "回転12ヶ月には年間7.5億円の販売が必要となる。SPC在庫を優先して販売することで達成する。");

  const rows = [
    [th("在庫回転期間"), th("必要な年間販売額"), th("年間税前利益"), th("投資家取分（60%）"), th("エクイティ定常年間利回り"), th("位置づけ")],
    [td("9ヶ月（最短）"), td("9.83億円"), td("2.71億円"), td("163百万円"), td("54.2%", { bold: true }), td("販路拡大が実現した場合の上限", { align: "left" })],
    [td("12ヶ月（主線）", { bold: true, color: GOLD_L }), td("7.48億円", { bold: true, color: GOLD_L }), td("2.10億円"), td("126百万円"), td("42.0%", { bold: true, color: GOLD_L }), td("SPC在庫優先売却により達成する水準", { align: "left", color: GOLD_L })],
    [td("15ヶ月"), td("6.07億円"), td("1.73億円"), td("104百万円"), td("34.7%", { bold: true }), td("現状の販売実力の範囲内", { align: "left" })],
    [td("18ヶ月"), td("5.13億円"), td("1.49億円"), td("89百万円"), td("29.8%", { bold: true }), td("下振れ時の想定レンジ下限", { align: "left" })],
  ];
  table(s, rows, M, 1.80, CW, [2.00, 2.05, 1.75, 1.90, 2.20, 1.99], { rowH: 0.42 });

  const cw3 = (CW - 0.28 * 2) / 3;
  const plan = [
    ["現状の販売実力", "5〜7億円", "楽天・Yahoo・寺田Wine MarketでのEC、酒販店卸、グループ飲食による現状の年間販売額。回転12ヶ月に必要な7.5億円は、この上限をやや上回る水準にある。", false],
    ["ギャップを埋める第一の手段", "SPC在庫の優先売却", "WineBank自己勘定在庫より先にSPC在庫を売却する運用ルールを契約上の義務とする。現状の販売力をSPCに優先配分することで回転12ヶ月を達成する。", true],
    ["販売力そのものを伸ばす打ち手", "海外・オークション", "輸出入ライセンスを活かした海外販売、2026年9月開始のWineBank Auction、グループ飲食（アピシウス／ティエリーマルクス）。これらは出資金ではなくWineBank側の資本で手当てする。", false],
  ];
  plan.forEach((c, i) => {
  ...
  s.addText("※ 回転12ヶ月に必要な年間販売額7.48億円は、現状の販売実力（5〜7億円）の上限をやや上回る。この差はWineBank自己勘定在庫の販売を後回しにすることで埋める設計であり、SPC在庫の優先売却を契約上の義務として明記する（在庫配分の公平性の頁を参照）。エクイティ利回りが20%を割り込むのは回転が30ヶ月を超えた場合。", {
    x: M, y: 6.22, w: CW, h: 0.44, margin: 0, fontFace: SANS, fontSize: 8.5, color: DIM, lineSpacingMultiple: 1.2 });

  s.addNotes("回転12ヶ月＝年間7.48億円。現状実力の上限をやや上回るため、SPC優先売却で埋める。");
}
...
// ═══════════════════════════════════════════════════════════ 09 ベースケース表
{
  const s = chrome(base(), "BASE CASE ｜ 主線", "ニュートラル：在庫回転期間別シミュレーション",
    "SPC取得原価50.5・ワイン価格上昇 年6%／総額5億円（デット2億＋エクイティ3億）・投資家取分60%・稼働率95%");

  const rows = [
    [th("在庫回転期間"), th("年回転数"), th("年間売上"), th("年間粗利"), th("SPC費用"), th("年間税前利益"), th("投資家取分（60%）"), th("エクイティ定常年間利回り"), th("5年累計分配"), th("年平均利回り 5年通算")],
    [td("9ヶ月"), td("1.33回"), td("9.83億"), td("3.49億"), td("78百万"), td("2.71億"), td("163百万"), td("54.2%", { bold: true }), td("730百万"), td("48.7%", { bold: true, color: GOLD_L })],
    [td("12ヶ月（主線）", { bold: true, color: GOLD_L }), td("1.00回"), td("7.48億"), td("2.73億"), td("63百万"), td("2.10億"), td("126百万"), td("42.0%", { bold: true, color: GOLD_L }), td("502百万"), td("33.5%", { bold: true, color: GOLD_L })],
    [td("15ヶ月"), td("0.80回"), td("6.07億"), td("2.27億"), td("54百万"), td("1.73億"), td("104百万"), td("34.7%", { bold: true }), td("387百万"), td("25.8%", { bold: true, color: GOLD_L })],
    [td("18ヶ月"), td("0.67回"), td("5.13億"), td("1.97億"), td("48百万"), td("1.49億"), td("89百万"), td("29.8%", { bold: true }), td("400百万"), td("26.7%", { bold: true })],
  ];
  ...
  s.addText("主線の回転12ヶ月で42.0%。最短9ヶ月なら54.2%、18ヶ月まで延びても29.8%を確保する。20%を割り込むのは回転が30ヶ月を超えた場合であり、想定レンジの外にある。", {
...
// ═══════════════════════════════════════════════════════════ 10 回転とリターン
{
  const s = chrome(base(), "BASE CASE ｜ 主線", "在庫回転期間と投資家リターンの関係",
    "主線は12ヶ月。回転が延びても値上がりが下支えするため、18ヶ月でも29.8%を確保する。");

  const cats = ["9ヶ月", "12ヶ月", "15ヶ月", "18ヶ月"];
  const specs = [
    ["エクイティ定常年間利回り（％）", [54.2, 42.0, 34.7, 29.8], 62, GOLD],
    ["年平均利回り（5年通算・％）", [48.7, 33.5, 25.8, 26.7], 56, GOLD_D],
  ];
  ...
  const notes = [
    ["回転期間が主たる決定要因", "回転を1ヶ月短縮するごとにエクイティ利回りは約4ポイント改善する。値引きや粗利率の微調整より、売り切る速度を上げる打ち手のほうが効く。"],
...
// ═══════════════════════════════════════════════════════════ 12 上振れ・下振れ
{
  ...
  table(s, [
    [th("回転期間"), th("必要年間売上"), th("年間税前"), th("投資家取分"), th("エクイティ利回り")],
  ...
  s.addText("「回転を上げるために値引く」は割に合わない。売値5%の値引きは単位粗利を15%毀損し、失うエクイティ利回り7.0ポイントを取り戻すには回転を12ヶ月から約10ヶ月へ短縮する必要がある。価格を維持したまま販売チャネルを増やすことが本筋の改善手段であり、むしろ売れ残りは値上がりを取りにいく機会でもある。", {
...
// ═══════════════════════════════════════════════════════════ 12 3シナリオ比較
{
  ...
  const cols = [
    ["主線の提示レンジ", "回転12ヶ月でエクイティ利回り42.0%。想定レンジ9〜18ヶ月では29.8〜54.2%となる。", true],
...
// ═══════════════════════════════════════════════════════════ 13 感応度
{
  const s = chrome(base(), "SENSITIVITY", "感応度：何が崩れると利回りがどう動くか",
    "主線（在庫回転12ヶ月・エクイティ利回り42.0%）を基準とした、各変数の単独変動による影響。");
  ...
  s.addText("結論：エクイティ利回りが20%を割り込むのは、回転が30ヶ月を超えた場合のみ（ニュートラル前提）で、想定レンジ9〜18ヶ月の外にある。ただし値引き5%と価格横ばいが同時に起きる複合的な下振れ（ネガティブ）では、回転15.9ヶ月が20%の分岐点となり想定レンジの中に入る。なお、インポーターの仕入価格が上昇する局面では希望小売価格も連動して上昇するため、定価比の構造は維持され、利回りへの影響は生じない。", {
...
// ═══════════════════════════════════════════════════════════ 在庫配分の公平性
{
  const s = chrome(base(), "FAIR ALLOCATION", "WineBank在庫との公平性をどう担保するか",
    "調達・保管・販売のすべてをWineBankが担うため配分に構造的な利益相反が生じる。SPC在庫の優先売却を第一原則とする。");

  const cw = (CW - 0.28 * 2) / 3;
  const layers = [
    ["01", "SPC在庫の優先売却", "同一銘柄が両者にある場合、当該SKUのSPC在庫が完売するまでWineBankは自己勘定在庫を売却しない。回転12ヶ月を実現するための運用方針であると同時に、配分の公平性を担保する第一の仕組みでもある。契約上の義務として明記する。", true],
    ["02", "銘柄排他の併用", "SKU（銘柄×ヴィンテージ×フォーマット）単位で、可能な限りSPC組入銘柄とWineBank自己勘定在庫を重複させない。買い手が銘柄を指名した時点で出所が一意に決まるため、配分の判断自体が発生しない。", false],
  ...
  s.addText("※ SPC在庫の優先売却は投資家保護のための譲歩であると同時に、回転12ヶ月という運用目標を達成するための手段でもある。WineBank自己勘定在庫の販売は後回しとなるが、ファンド期間中はこれを受け入れる。組成前に組入予定銘柄と自己勘定在庫の重複度を精査し、02の適用範囲を確定する。", {
...
// ═══════════════════════════════════════════════════════════ 15 条件
{
  ...
  const right = [
    ...
    [td("想定リターン"), td("エクイティ定常年間利回り42.0%（在庫回転12ヶ月）。5年通算の年平均利回りは33.5%", { align: "left", color: GOLD_L, bold: true })],
    [td("報告"), td("四半期：在庫明細・簿価・売上・試算表／年次：決算", { align: "left" })],
    [td("外部検証"), td("AUP（合意された手続）を年次で実施。配分ルールの遵守状況を手続の対象に含める", { align: "left" })],
    [td("在庫配分"), td("SPC在庫の優先売却を第一原則とする。契約別紙として明文化する", { align: "left", color: GOLD_L, bold: true })],
  ];
  ...
  s.addText("半期解約を前提とすると常時1〜3割を現金または即時換金可能な在庫で保有する必要があり、稼働率が下がって利回りも下がる（稼働率95%→85%でエクイティ利回りは42.0%→37.4%）。当初3年ロックアップ・4年目以降は年度ごとの解約という設計により、現金保有を5%に抑え、総額の95%をワインに充当できる。", {
...
// ═══════════════════════════════════════════════════════════ 17 まとめ
{
  ...
  const merits = [
    ...
    ["収益が二階建てで、片方が崩れても成立する",
     "流通価格差（回転に対するリターン）とワインの値上がり（在庫に対するリターン）。価格が横ばいでもエクイティ利回り34.1%、回転が18ヶ月に延びても29.8%を確保する。"],
    ["他社が再現できない調達力",
     "創業55年の酒販免許、大手インポーターとの直接取引、輸出入ライセンス。新規参入業者が正規品を同じ価格で大量に仕入れることはできない。"],
    ["出口をB2B・B2Cの両輪で持ち、SPC在庫を先に売る",
     "酒販店卸（B2B）に加え、自社EC・オークション・CLUB会員・グループのアピシウス（B2C）。そのうえでWineBank自己勘定在庫よりSPC在庫を優先して売却する。"],
    ["運営者と利害が完全に一致している",
  ...
  const checks = [
    ["想定利回り20%（実績）に対して",
     "在庫回転12ヶ月を主線とし、エクイティ利回り42.0%。想定レンジ9〜18ヶ月では29.8〜54.2%となり、いずれも貴社基準を上回る。",
     "SPC在庫を優先して売却することで回転12ヶ月を実現する。回転期間を運用KPIとして四半期報告する。"],
    ["解約条件に対して",
