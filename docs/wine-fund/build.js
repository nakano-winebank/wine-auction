...
// ═══════════════════════════════════════════════════════════ 02 サマリー
{
  ...
  stat(s, M + (tw + 0.30) * 1, 1.82, tw, 1.62, "26.2", "%", "想定粗利率（主線）", "手取り70.2・SPC取得原価50.5・単位粗利19.7");
  stat(s, M + (tw + 0.30) * 2, 1.82, tw, 1.62, "17.1", "%", "定常年間利回り", "回転12ヶ月・投資家取分ベースの実力値");
  stat(s, M + (tw + 0.30) * 3, 1.82, tw, 1.62, "13.7–20.9", "%", "年平均利回り（5年通算）", "回転12ヶ月〜9ヶ月／税引前",
    { vs: 24, fill: CARD2, line: GOLD_D });

  const pts = [
    ["リターンを決めるのは粗利率ではなく在庫回転期間。",
     "粗利率は前提として固定されるため、投資家リターンは「在庫を年に何回まわせるか」でほぼ決まる。回転9ヶ月で定常利回り23.3%、12ヶ月で17.1%。"],
    ["販売はB2B卸とB2Cネット販売の半々。手取りではB2Cが上回る。",
     "B2B（酒販店卸）は売値70で手数料が軽く手取り69.3。B2C（自社EC・オークション）は売値80だが手数料11.2%を負い手取り71.0。両者を半々で運用する。"],
    ["出資金の95%を現物ワインに充当する。",
     "当初3年間をロックアップとすることで現金保有を5%に抑えられる。値上がり益は一切リターンに算入せず、流通価格差のみを収益源とする保守設計。"],
...
// ═══════════════════════════════════════════════════════════ 05 収益の源泉
{
  ...
  s.addText("主線（ニュートラル）の置き方", { x: M + 0.30, y: 4.18, w: 5, h: 0.28, margin: 0, fontFace: SANS, fontSize: 12, bold: true, color: GOLD_L });

  const calc = [
    ["仕入ポートフォリオ", "インポーター40 ＋ 酒販店60 を半々", "市中原価 50.00", false],
    ["現物譲渡（WineBank→SPC）", "独立企業間価格として1%を付加", "SPC取得原価 50.50", false],
    ["売却ポートフォリオ", "酒販店卸70（B2B）＋ ネット最安80（B2C）を半々", "売値 75.00", false],
    ["変動販売費", "B2B 1.0% ／ B2C 11.2% の加重平均 6.44%", "▲ 4.83 → 手取り 70.17", false],
    ["単位粗利", "手取り70.17 − SPC取得原価50.50", "粗利 19.67（粗利率 26.2%）", true],
  ];
  let cy = 4.54;
  calc.forEach((c) => {
  ...
  s.addText("副次（ポジティブ）：仕入はインポーター40のみ・売却はネット最安80のみ（B2C100%）→ SPC取得原価40.4・手取り71.0・粗利30.6（粗利率38.3%）。調達枠の確保が前提条件。", {
    x: M, y: 6.26, w: CW, h: 0.28, margin: 0, fontFace: SANS, fontSize: 9, color: DIM,
  });
  s.addNotes("会社紹介資料17ページの流通価格構造をそのまま採用。売値75は変動販売費控除前の数字である点に注意。");
}
...
// ═══════════════════════════════════════════════════════════ 06 チャネル別の単位経済
{
  ...
  s.addText([
    ...
    { text: "　B2Cは手数料率11.2%を負うが、売値が定価比80とB2Bの70を10ポイント上回るため、差引きで勝る。ただしB2Cは1本ずつの出荷となり販売量の上限が低いため、回転期間を確保するにはB2Bの卸売が不可欠となる。両者を半々で運用し、加重平均で売値75.00・変動販売費6.44%・手取り70.17を主線とする。", options: { fontFace: SANS, fontSize: 10, color: IVORY } },
...
// ═══════════════════════════════════════════════════════════ 07 3シナリオの単位経済
{
  const s = chrome(base(), "UNIT ECONOMICS", "3シナリオの単位経済（定価100あたり）",
    "現物譲渡の1%とチャネル別の変動販売費を、全シナリオで控除したうえでの粗利。");

  const sc = [
    ["ネガティブ", ["50.00", "50.50", "71.25", "4.59", "66.66", "16.16", "22.7%"], "売値を5%値引き（B2B66.5／B2C76）", LINE, false],
    ["ニュートラル（主線）", ["50.00", "50.50", "75.00", "4.83", "70.17", "19.67", "26.2%"], "仕入40/60の半々・売却はB2B70とB2C80の半々", GOLD_D, true],
    ["ポジティブ", ["40.00", "40.40", "80.00", "8.96", "71.04", "30.64", "38.3%"], "仕入はインポーター40のみ・売却はネット最安80のみ（B2C100%）", LINE, false],
  ];
  const labels = ["市中仕入原価", "SPC取得原価（＋1%）", "売値（加重平均）", "変動販売費", "手取り", "単位粗利", "粗利率"];
  ...
  sc.forEach((c, i) => {
    ...
    let yy = 2.48;
    c[1].forEach((v, j) => {
      const hot = (j === 1 || j === 4 || j === 5 || j === 6);
      s.addText(labels[j], { x: x + 0.28, y: yy, w: cw - 1.60, h: 0.28, margin: 0, valign: "middle", fontFace: SANS, fontSize: 10, color: MUTED });
      s.addText(j === 3 ? "▲ " + v : v, {
        x: x + cw - 1.62, y: yy, w: 1.34, h: 0.28, margin: 0, align: "right", valign: "middle",
        fontFace: LATIN, fontSize: j === 6 ? 13 : 14, bold: true,
        color: j === 3 ? RED : (hot ? (c[4] ? GOLD_L : IVORY) : IVORY),
      });
      yy += 0.42;
    });
    s.addText(c[2], {
      x: x + 0.28, y: 5.34, w: cw - 0.56, h: 0.40, margin: 0,
      fontFace: SANS, fontSize: 9, color: DIM, lineSpacingMultiple: 1.2,
    });
  });

  s.addText("※ 交渉余地を織り込み、ニュートラルを提案の主線とする。ポジティブは調達・販売チャネルを最適化した場合の上振れ余地として位置づける。粗利率は変動販売費控除後の手取りに対する比率であり、売値に対する見かけの粗利率（33.3%）より低く出る。", {
    x: M, y: 5.98, w: CW, h: 0.40, margin: 0, fontFace: SANS, fontSize: 9, color: DIM, lineSpacingMultiple: 1.2,
  });
  s.addNotes("粗利率26.2%は変動販売費控除後。売値ベースの見かけの粗利率33.3%と混同しないこと。");
}
...
// ═══════════════════════════════════════════════════════════ 08 費用前提
{
  ...
  s.addText("※ 保管料は業務用パレット定温保管の水準（月10〜20円／本）。個人向けセラー預かり（月100〜150円／本）とは別の料金帯。年間SPC費用の合計は5,910万円（売上比8.4%）で、内訳は変動販売費4,543万円・保管705万円・SPC固定費560万円・保険71万円・入庫30万円。その他の運用前提：出資金の95%をワイン購入に充当（現金5%）／仕入展開6ヶ月／ローリング運用／利益は複利再投資せず半期分配／税前利益を投資家50：WineBank50で折半／利回りは投資家の税引前ベース。", {
...
// ═══════════════════════════════════════════════════════════ 09 ベースケース表
{
  const s = chrome(base(), "BASE CASE ｜ 主線", "ニュートラル：在庫回転期間別シミュレーション",
    "SPC取得原価50.5・手取り70.2・粗利19.7（粗利率26.2%）／投資枠5億円・運用期間5年・稼働率95%");

  const rows = [
    [th("在庫回転期間"), th("年回転数"), th("年間売上"), th("年間粗利"), th("SPC費用"), th("年間税前利益"), th("投資家配当／年"), th("定常年間利回り"), th("5年累計分配"), th("年平均利回り 5年通算")],
    [td("9ヶ月"), td("1.33回"), td("9.41億"), td("3.07億"), td("74百万"), td("2.33億"), td("116百万"), td("23.3%", { bold: true }), td("523百万"), td("20.9%", { bold: true, color: GOLD_L })],
    [td("12ヶ月", { bold: true, color: GOLD_L }), td("1.00回"), td("7.05億"), td("2.30億"), td("59百万"), td("1.71億"), td("86百万"), td("17.1%", { bold: true }), td("341百万"), td("13.7%", { bold: true, color: GOLD_L })],
    [td("15ヶ月"), td("0.80回"), td("5.64億"), td("1.84億"), td("50百万"), td("1.34億"), td("67百万"), td("13.4%", { bold: true }), td("250百万"), td("10.0%", { bold: true, color: GOLD_L })],
    [td("18ヶ月"), td("0.67回"), td("4.70億"), td("1.54億"), td("44百万"), td("1.10億"), td("55百万"), td("11.0%", { bold: true }), td("246百万"), td("9.8%", { bold: true })],
  ];
  ...
  const read = [
    ...
    "回転期間が短いほど改善幅は加速（15→12ヶ月で＋3.7pt、12→9ヶ月で＋6.2pt）",
  ];
  let ry = 4.58;
  read.forEach((t) => {
...
// ═══════════════════════════════════════════════════════════ 10 回転とリターン
{
  ...
  const specs = [
    ["定常年間利回り（％）", [23.3, 17.1, 13.4, 11.0], 27, GOLD],
    ["年平均利回り（5年通算・％）", [20.9, 13.7, 10.0, 9.8], 25, GOLD_D],
  ];
  ...
  const notes = [
    ["回転期間がすべて", "回転12ヶ月を1ヶ月短縮するごとに定常年間利回りは約1.7ポイント改善する。値引きや粗利率の微調整より、回転を上げる打ち手のほうが効く。"],
    ["運用期間5年が回転の遅れを吸収する", "展開6ヶ月＋回転18ヶ月でも5年間で3回転できる。3年満期では1回転しかできず資金が1年以上遊ぶため、5年設計が回転期間のブレに対する耐性を生んでいる。"],
  ];
  notes.forEach((n, i) => {
...
// ═══════════════════════════════════════════════════════════ 11 上振れ・下振れ
{
  const s = chrome(base(), "UPSIDE / DOWNSIDE ｜ 副次", "上振れ余地と下振れリスク",
    "調達・販売チャネルの寄せ方で上振れし、値引き販売で下振れする。いずれも譲渡1%・変動販売費控除後の数値。");

  card(s, M, 1.80, CW / 2 - 0.15, 2.76, { fill: CARD2, line: GOLD_D });
  s.addText("ポジティブ｜SPC取得原価40.4・B2C100%・粗利率38.3%", {
  ...
  table(s, [
    ...
    [td("9ヶ月"), td("12.54億"), td("4.65億"), td("46.5%"), td("41.8%", { bold: true, color: GOLD_L })],
    [td("12ヶ月"), td("9.41億"), td("3.45億"), td("34.5%"), td("27.5%", { bold: true, color: GOLD_L })],
    [td("15ヶ月"), td("7.52億"), td("2.73億"), td("27.3%"), td("20.4%")],
    [td("18ヶ月"), td("6.27億"), td("2.25億"), td("22.5%"), td("20.2%")],
  ...
  s.addText("ネガティブ｜売値5%値引き・粗利率22.7%", {
  ...
  s.addText("売値を5%（B2B70→66.5／B2C80→76）値引きすると定常年間利回りは3.3ポイント低下する。単位粗利19.67に対しては19%のインパクトとなる。", {
  ...
  s.addText("「回転を上げるために値引く」は割に合わない。売値5%の値引きは単位粗利を19%毀損し、失う利回り3.3ポイントを取り戻すには回転期間を12ヶ月から約10ヶ月へ短縮する必要がある。価格を維持したまま販売チャネルを増やすことが本筋の改善手段となる。", {
...
// ═══════════════════════════════════════════════════════════ 12 3シナリオ比較
{
  ...
  s.addChart(pres.ChartType.bar, [
    { name: "ポジティブ", labels: cats, values: [46.5, 34.5, 27.3, 22.5] },
    { name: "ニュートラル（主線）", labels: cats, values: [23.3, 17.1, 13.4, 11.0] },
  ...
  const cols = [
    ["主線の提示レンジ", "回転9〜12ヶ月で定常年間利回り17.1〜23.3%（5年通算の年平均利回り13.7〜20.9%）。保守的な前提を積み上げた数値であり、交渉の出発点として提示する。", true],
    ["上振れの条件", "調達をインポーター直に寄せ、販売をネット最安値帯に寄せれば、同じ回転期間で利回りは2倍になる。", false],
    ["下振れの限界", "値引き5%を強いられても、回転9ヶ月を維持できれば定常利回り18.9%を確保できる。回転の維持が防衛線。", false],
  ];
...
// ═══════════════════════════════════════════════════════════ 13 感応度
{
  const s = chrome(base(), "SENSITIVITY", "感応度：何が崩れると利回りがどう動くか",
    "ニュートラル・回転12ヶ月（定常年間利回り17.1%）を基準とした、各変数の単独変動による影響。");

  const rows = [
    [th("変動要因"), th("変動幅"), th("定常年間利回りへの影響"), th("深刻度"), th("対応方針")],
    [td("在庫回転期間"), td("12ヶ月 → 18ヶ月"), td("17.1% → 11.0%（▲6.2pt）", { color: RED, bold: true }), td("最大", { color: RED, bold: true }), td("販売チャネル多重化・売れ筋銘柄への集中", { align: "left" })],
    [td("売却価格"), td("売値 ▲5%（70/80→66.5/76）"), td("17.1% → 13.8%（▲3.3pt）", { color: RED }), td("大"), td("値引き販売を禁じ、価格維持を運用ルール化", { align: "left" })],
    [td("稼働率"), td("95% → 85%"), td("17.1% → 15.3%（▲1.9pt）", { color: RED }), td("中"), td("3年経過後の解約に備えた現金比率の設計と表裏", { align: "left" })],
    [td("B2Cのモール手数料"), td("5.5% → 8.0%"), td("17.1% → 16.2%（▲0.9pt）", { color: MUTED }), td("小"), td("自社EC・自社オークションの構成比を引き上げる", { align: "left" })],
    [td("販売チャネル構成"), td("B2B50:B2C50 → B2C100%"), td("17.1% → 18.0%（＋0.9pt）", { color: GOLD_L }), td("上振れ", { color: GOLD_L }), td("手数料は重いが売値の高さが上回る。ただし出荷量に上限", { align: "left" })],
    [td("調達構成"), td("定価比 50 → 45"), td("17.1% → 24.4%（＋7.3pt）", { color: GOLD_L, bold: true }), td("上振れ", { color: GOLD_L, bold: true }), td("インポーター直取引比率の引き上げが最大の改善策", { align: "left" })],
  ];
  table(s, rows, M, 1.82, CW, [1.96, 2.42, 2.58, 0.96, 3.97], { rowH: 0.42 });

  card(s, M, 5.10, CW, 1.10, { fill: CARD2, line: GOLD_D });
  s.addText("結論：リターン改善の打ち手は「回転期間の短縮」と「調達構成の改善」の2つに集約される。なお、インポーターの仕入価格が上昇する局面では希望小売価格も連動して上昇するため、定価比の構造（仕入50・売却70／80）は維持され、利回りへの影響は生じない。", {
    x: M + 0.30, y: 5.24, w: CW - 0.60, h: 0.82, margin: 0, valign: "middle",
    fontFace: SANS, fontSize: 11.5, color: IVORY, lineSpacingMultiple: 1.25 });
  s.addNotes("回転と調達構成が本丸。仕入価格の上昇は希望小売も連動するため中立。");
}
...
// ═══════════════════════════════════════════════════════════ 15 条件
{
  ...
  const rows = [
    ...
    [td("想定リターン"), td("定常年間利回り 17.1〜23.3%（回転12〜9ヶ月）", { align: "left" }), td("ニュートラル前提。5年通算の年平均利回りは13.7〜20.9%", { align: "left" })],
  ...
  s.addText("半期解約を前提とすると常時1〜3割を現金または即時換金可能な在庫で保有する必要があり、稼働率が下がって利回りも下がる（稼働率95%→85%で定常年間利回りは17.1%→15.3%）。当初3年ロックアップ・4年目以降は年度ごとの解約という設計により、現金保有を5%に抑え、出資金の95%をワインに充当できる。", {
...
// ═══════════════════════════════════════════════════════════ 17 まとめ
{
  ...
  const merits = [
    ["現物が元本を裏付ける",
     "出資金の95%が常にワイン現物として存在する。SPC名義で保有し倒産隔離。値上がり益はリターンに一切算入していない。"],
    ["相場ではなく流通の価格差から収益を得る",
  ...
  const checks = [
    ["想定利回り20%（実績）に対して",
     "定常年間利回りが20%に到達するのは在庫回転10.4ヶ月。回転9ヶ月なら23.3%、12ヶ月で17.1%。",
     "回転10ヶ月前後が貴社基準との分岐点。ここを運用KPIとして四半期報告する。"],
    ["解約条件に対して",
     "運用期間5年・当初3年ロックアップ・4年目以降は年度ごとの解約。",
     "この設計により現金保有を5%に抑え、出資金の95%をワインに充当できる。半期解約を前提とすると稼働率が下がり17.1%→15.3%に低下する。"],
    ["リスク分散に対して",
