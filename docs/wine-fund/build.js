...
// ═══════════════════════════════════════════════════════════ 02 サマリー
{
  ...
  stat(s, M + (tw + 0.30) * 2, 1.82, tw, 1.62, "35.0", "%", "投資家利回り（定常年間）", "出資3億円に対し、SPC税前利益の50%を受領");
  stat(s, M + (tw + 0.30) * 3, 1.82, tw, 1.62, "27.9", "%", "年平均利回り（5年通算）", "在庫回転12ヶ月／累計分配額÷出資3億円÷5年",
  ...
  const pts = [
    ...
    ["損益は従来どおり折半。投資家は出資3億円に対しSPC税前利益の50%を受け取る。",
     "デット2億円の金利と保証責任はWineBankが負担する。投資家利回り35.0%は、粗利率30.0%を投下原価ベースに直した投下原価利益率47.3%から、SPC費用と折半を経て導かれる。"],
...
// ═══════════════════════════════════════════════════════════ 03 資本構成
{
  ...
  const tranches = [
    ...
    ["出資｜投資家", "3.0", "億円", "SPC税前利益の50%", ["匿名組合出資", "満期に元本償還", "半期ごとに分配"], true],
    ["GP｜WineBank", "97", "百万円", "SPC税前利益の50%（金利負担後の年間取分）", ["現金拠出はなし", "デット2億の金利と保証責任を負担", "期中報酬・成功報酬は取らない"], false],
  ];
  ...
  const rows = [
    ...
    [td("├ 投資家 50%"), td("105百万円", { bold: true, color: GOLD_L }), td("出資3億円に対し 投資家利回り 35.0%", { align: "left", color: GOLD_L, bold: true })],
    [td("└ WineBank 50%"), td("105百万円"), td("デット2億円の金利8百万円を負担 → 年間取分97百万円。現金拠出はないため利回りの形では示さない", { align: "left" })],
  ...
  s.addText("損益の配分は従来どおりSPC税前利益の折半であり、この点は変更していない。投資家は出資3億円に対して税前利益の50%を受け取る。デット2億円はWineBankが金利と保証責任を負う借入であり、投資家の出資とは切り離されている。WineBankは現金を拠出しないため、その取分は金額（年間97百万円）で示す。WineBankは期中報酬も成功報酬も取らず、報酬はこの利益折半に一本化されている。", {
...
// ═══════════════════════════════════════════════════════════ 04 販売計画
{
  ...
  const rows = [
    [th("在庫回転期間"), th("必要な年間販売額"), th("年間税前利益"), th("投資家取分（50%）"), th("投資家利回り"), th("WineBank取分"), th("位置づけ")],
    [td("9ヶ月（最短）"), td("9.83億円"), td("2.71億円"), td("136百万円"), td("45.2%", { bold: true, color: GOLD_L }), td("128百万円"), td("販路拡大が実現した場合の上限", { align: "left" })],
    [td("12ヶ月（主線）", { bold: true, color: GOLD_L }), td("7.48億円", { bold: true, color: GOLD_L }), td("2.10億円"), td("105百万円"), td("35.0%", { bold: true, color: GOLD_L }), td("97百万円"), td("組入銘柄の選定と配分ルールで達成", { align: "left", color: GOLD_L })],
    [td("15ヶ月"), td("6.07億円"), td("1.73億円"), td("87百万円"), td("28.9%", { bold: true, color: GOLD_L }), td("79百万円"), td("現状の販売実力の範囲内", { align: "left" })],
    [td("18ヶ月"), td("5.13億円"), td("1.49億円"), td("74百万円"), td("24.8%", { bold: true, color: GOLD_L }), td("66百万円"), td("下振れ時の想定レンジ下限", { align: "left" })],
  ];
  ...
  s.addText("※ 回転12ヶ月に必要な年間販売額7.48億円は、現状の販売実力（5〜7億円）の上限をやや上回る。銘柄の大半はSPCと自己勘定で重複しないため配分の判断自体が生じず、回転期間は「何を組み入れるか」でほぼ決まる。重複銘柄の取扱いは在庫配分の公平性の頁を参照。投資家利回りが20%を割り込むのは回転が23.7ヶ月を超えた場合。", {
...
// ═══════════════════════════════════════════════════════════ 09 ベースケース表
{
  const s = chrome(base(), "BASE CASE ｜ 主線", "ニュートラル：在庫回転期間別シミュレーション",
    "SPC取得原価50.5・ワイン価格上昇 年6%／総額5億円（デット2億＋出資3億）・損益は折半・稼働率95%");

  const rows = [
    [th("在庫回転期間"), th("年間売上"), th("年間粗利※"), th("粗利率"), th("その他SPC費用"), th("年間税前利益"), th("投資家取分（50%）"), th("投資家利回り"), th("5年累計分配"), th("投資家 年平均利回り 5年通算")],
    [td("9ヶ月"), td("9.83億"), td("2.86億"), td("29.1%"), td("15百万"), td("2.71億"), td("136百万"), td("45.2%", { bold: true, color: GOLD_L }), td("609百万"), td("40.6%", { bold: true, color: GOLD_L })],
    [td("12ヶ月（主線）", { bold: true, color: GOLD_L }), td("7.48億"), td("2.25億"), td("30.0%"), td("15百万"), td("2.10億"), td("105百万"), td("35.0%", { bold: true, color: GOLD_L }), td("418百万"), td("27.9%", { bold: true, color: GOLD_L })],
    [td("15ヶ月"), td("6.07億"), td("1.88億"), td("31.0%"), td("15百万"), td("1.73億"), td("87百万"), td("28.9%", { bold: true, color: GOLD_L }), td("323百万"), td("21.5%", { bold: true, color: GOLD_L })],
    [td("18ヶ月"), td("5.13億"), td("1.64億"), td("31.9%"), td("15百万"), td("1.49億"), td("74百万"), td("24.8%", { bold: true, color: GOLD_L }), td("333百万"), td("22.2%", { bold: true })],
  ];
  table(s, rows, M, 1.82, CW, [1.62, 0.98, 0.98, 0.82, 1.28, 1.20, 1.32, 1.16, 1.10, 1.43], { rowH: 0.40 });

  card(s, M, 4.06, 6.05, 1.98, { fill: CARD2, line: GOLD_D });
  s.addText("粗利率30.0%が投資家利回り35.0%になる理由", { x: M + 0.28, y: 4.20, w: 5.5, h: 0.26, margin: 0, fontFace: SANS, fontSize: 11.5, bold: true, color: GOLD_L });
  const bridge = [
    ...
    ["税前利回り 42.0%", "SPC費用（保管・保険・入庫・固定 約1,500万円）控除後／総資産5億円に対して", false],
    ["投資家利回り 35.0%", "税前利益の50%（105百万円）を出資3億円で除したもの", true],
  ];
  let by = 4.52;
  bridge.forEach((b) => {
  ...
  s.addText([
    { text: "24.8 – 45.2", options: { fontFace: LATIN, fontSize: 30, bold: true, color: GOLD_L } },
    { text: " %", options: { fontFace: SANS, fontSize: 15, bold: true, color: GOLD_L } },
  ], { x: M + 6.63, y: 4.78, w: CW - 6.95, h: 0.50, margin: 0, valign: "middle" });
  s.addText("主線の回転12ヶ月で35.0%。最短9ヶ月なら45.2%、18ヶ月まで延びても24.8%を確保する。20%を割り込むのは回転が23.7ヶ月を超えた場合であり、想定レンジの外にある。", {
...
// ═══════════════════════════════════════════════════════════ 10 回転とリターン
{
  const s = chrome(base(), "BASE CASE ｜ 主線", "在庫回転期間と投資家リターンの関係",
    "いずれも出資3億円に対する数値。SPC税前利益の50%を投資家が受け取る前提。");

  const cats = ["9ヶ月", "12ヶ月", "15ヶ月", "18ヶ月"];
  const cwid = (CW - 0.32) / 2;
  card(s, M, 1.80, cwid, 2.86);
  s.addChart(pres.ChartType.bar, [{ name: "投資家利回り", labels: cats, values: [45.2, 35.0, 28.9, 24.8] }],
    Object.assign({}, CHART_BASE, {
      x: M + 0.14, y: 1.90, w: cwid - 0.28, h: 2.66,
      barDir: "col", barGapWidthPct: 55,
      title: "投資家利回り（定常年間・％）", valAxisMaxVal: 52,
      chartColors: [GOLD],
      plotArea: { fill: { color: CARD } }, chartArea: { fill: { color: CARD } },
    }));
  card(s, M + cwid + 0.32, 1.80, cwid, 2.86);
  s.addChart(pres.ChartType.bar, [{ name: "投資家 年平均利回り", labels: cats, values: [40.6, 27.9, 21.5, 22.2] }],
    Object.assign({}, CHART_BASE, {
      x: M + cwid + 0.46, y: 1.90, w: cwid - 0.28, h: 2.66,
      barDir: "col", barGapWidthPct: 55,
      title: "投資家 年平均利回り（5年通算・％）", valAxisMaxVal: 46,
      chartColors: [GOLD_D],
      plotArea: { fill: { color: CARD } }, chartArea: { fill: { color: CARD } },
    }));

  const notes = [
    ["回転期間が主たる決定要因", "回転を1ヶ月短縮するごとに投資家利回りは約3.4ポイント改善する。何を組み入れるか（銘柄の流動性）で回転期間の大半が決まる。"],
    ["ただし値上がりが下支えする", "回転が18ヶ月まで延びても24.8%を確保できる。保有が長いぶん売値が上がるためで、値上がりを織り込まない場合とは約6.6ポイントの差がつく（次頁）。"],
  ];
  notes.forEach((n, i) => {
...
// ═══════════════════════════════════════════════════════════ 11 値上がりの織り込み
{
  ...
  const rows = [
    ...
    [td("9ヶ月"), td("×1.045"), td("38.7%"), td("＋6.6pt", { color: GOLD_L }), td("45.2%", { bold: true, color: GOLD_L })],
    [td("12ヶ月（主線）", { bold: true, color: GOLD_L }), td("×1.060"), td("28.4%"), td("＋6.6pt", { color: GOLD_L }), td("35.0%", { bold: true, color: GOLD_L })],
    [td("15ヶ月"), td("×1.076"), td("22.2%"), td("＋6.6pt", { color: GOLD_L }), td("28.9%", { bold: true, color: GOLD_L })],
    [td("18ヶ月"), td("×1.091"), td("18.1%"), td("＋6.7pt", { color: GOLD_L }), td("24.8%", { bold: true, color: GOLD_L })],
  ];
  ...
  const rates = [["0%（横ばい）", "28.4%"], ["6%（主線）", "35.0%"], ["10%（会社資料水準）", "39.4%"]];
  rates.forEach((r, i) => {
  ...
  s.addText("値上がりは在庫金額に対して発生するため、年間の上乗せ幅は回転期間によらず約6.6ポイントで一定になる。回転が落ちた分をちょうど埋め合わせるわけではなく、どの回転期間でも一律に底上げする性質のもの。したがって回転期間の管理は引き続き最大の運用課題である。", {
    x: M + 6.61, y: 6.02, w: CW - 6.87, h: 0.68, margin: 0, fontFace: SANS, fontSize: 9, color: MUTED, lineSpacingMultiple: 1.2 });

  s.addNotes("値上がり効果は在庫に対するリターンなので回転期間によらず一定（約+6.6pt）。相殺ではなく底上げ。");
}
...
// ═══════════════════════════════════════════════════════════ 12 上振れ・下振れ
{
  ...
  table(s, [
    ...
    [td("9ヶ月"), td("13.10億"), td("5.13億"), td("257百万"), td("85.6%", { bold: true, color: GOLD_L })],
    [td("12ヶ月"), td("9.97億"), td("3.94億"), td("197百万"), td("65.6%", { bold: true, color: GOLD_L })],
    [td("15ヶ月"), td("8.09億"), td("3.22億"), td("161百万"), td("53.7%")],
    [td("18ヶ月"), td("6.84億"), td("2.75億"), td("137百万"), td("45.8%")],
  ...
  table(s, [
    [th("回転期間"), th("必要年間売上"), th("年間税前"), th("投資家取分"), th("投資家利回り")],
    [td("9ヶ月"), td("8.94億"), td("1.88億"), td("94百万"), td("31.3%", { bold: true })],
    [td("12ヶ月"), td("6.70億"), td("1.37億"), td("69百万"), td("22.9%", { bold: true })],
    [td("15ヶ月"), td("5.36億"), td("1.07億"), td("54百万"), td("17.8%", { color: RED })],
    [td("18ヶ月"), td("4.47億"), td("0.87億"), td("43百万"), td("14.5%", { color: RED })],
  ], nx + 0.28, 2.32, 5.23, [0.90, 1.05, 0.98, 0.98, 1.32], { rowH: 0.315, fontSize: 9.5 });
  s.addText("値引き（▲5.8pt）と価格横ばい（▲6.6pt）が同時に起きた複合ケース。この場合の20%の分岐点は回転13.5ヶ月となり、想定レンジの中に入る。最も厳しい防衛線となる。", {
  ...
  s.addText("「回転を上げるために値引く」は割に合わない。売値5%の値引きは単位粗利を15%毀損し、失う投資家利回り5.8ポイントを取り戻すには回転を12ヶ月から約10ヶ月へ短縮する必要がある。価格を維持したまま販売チャネルを増やすことが本筋の改善手段であり、むしろ売れ残りは値上がりを取りにいく機会でもある。", {
...
// ═══════════════════════════════════════════════════════════ 12 3シナリオ比較
{
  const s = chrome(base(), "COMPARISON", "3シナリオ比較：投資家 定常年間利回り",
    "出資3億円に対する投資家の年間実力値。SPC税前利益の50%を投資家が受け取る前提。");

  const cats = ["9ヶ月", "12ヶ月", "15ヶ月", "18ヶ月"];
  card(s, M, 1.80, CW, 2.98);
  s.addChart(pres.ChartType.bar, [
    { name: "ポジティブ", labels: cats, values: [85.6, 65.6, 53.7, 45.8] },
    { name: "ニュートラル（主線）", labels: cats, values: [45.2, 35.0, 28.9, 24.8] },
    { name: "ネガティブ", labels: cats, values: [31.3, 22.9, 17.8, 14.5] },
  ], Object.assign({}, CHART_BASE, {
  ...
  const cols = [
    ["主線の提示レンジ", "回転12ヶ月で投資家利回り35.0%。想定レンジ9〜18ヶ月では24.8〜45.2%となる。", true],
    ["上振れの条件", "調達をインポーター直に寄せた場合の水準。ただし回転12ヶ月で年間10億円の販売を要するため、現状は到達しない。上限の目安。", false],
    ["下振れの限界", "値引き5%と価格横ばいが重なった場合、20%の分岐点は回転13.5ヶ月。想定レンジの中に入るため最も厳しい防衛線となる。", false],
  ];
...
// ═══════════════════════════════════════════════════════════ 13 感応度
{
  const s = chrome(base(), "SENSITIVITY", "感応度：何が崩れると利回りがどう動くか",
    "主線（在庫回転12ヶ月・投資家利回り35.0%）を基準とした、各変数の単独変動による影響。");

  const rows = [
    [th("変動要因"), th("変動幅"), th("投資家利回りへの影響"), th("深刻度"), th("対応方針")],
    [td("在庫回転期間"), td("12ヶ月 → 18ヶ月"), td("35.0% → 24.8%（▲10.2pt）", { color: RED, bold: true }), td("最大", { color: RED, bold: true }), td("組入銘柄の選定・販売チャネルの多重化", { align: "left" })],
    [td("ワイン価格上昇率"), td("年6% → 0%（横ばい）"), td("35.0% → 28.4%（▲6.6pt）", { color: RED }), td("大", { color: RED }), td("回転期間を短縮し、価格差収益の比重を高める", { align: "left" })],
    [td("売却価格"), td("売値 ▲5%（70/80→66.5/76）"), td("35.0% → 29.2%（▲5.8pt）", { color: RED }), td("大"), td("値引き販売を禁じ、価格維持を運用ルール化", { align: "left" })],
    [td("稼働率"), td("95% → 85%"), td("35.0% → 31.2%（▲3.8pt）", { color: RED }), td("中"), td("3年経過後の解約に備えた現金比率の設計と表裏", { align: "left" })],
    [td("B2Cのモール手数料"), td("5.5% → 8.0%"), td("35.0% → 33.3%（▲1.7pt）", { color: MUTED }), td("小"), td("自社EC・自社オークションの構成比を引き上げる", { align: "left" })],
    [td("ワイン価格上昇率"), td("年6% → 10%（会社資料水準）"), td("35.0% → 39.4%（＋4.4pt）", { color: GOLD_L }), td("上振れ", { color: GOLD_L }), td("Liv-ex連動銘柄の構成比を高める", { align: "left" })],
    [td("調達構成"), td("定価比 50 → 45"), td("35.0% → 47.8%（＋12.8pt）", { color: GOLD_L, bold: true }), td("上振れ", { color: GOLD_L, bold: true }), td("インポーター直取引比率の引き上げが最大の改善策", { align: "left" })],
  ];
  table(s, rows, M, 1.80, CW, [1.96, 2.42, 2.58, 0.96, 3.97], { rowH: 0.39 });

  card(s, M, 5.14, CW, 1.10, { fill: CARD2, line: GOLD_D });
  s.addText("結論：投資家利回りが20%を割り込むのは、回転が23.7ヶ月を超えた場合（ニュートラル前提）で、想定レンジ9〜18ヶ月の外にある。ただし値引き5%と価格横ばいが同時に起きる複合的な下振れ（ネガティブ）では、回転13.5ヶ月が20%の分岐点となり想定レンジの中に入る。なお、インポーターの仕入価格が上昇する局面では希望小売価格も連動して上昇するため、定価比の構造は維持され、利回りへの影響は生じない。", {
    x: M + 0.30, y: 5.26, w: CW - 0.60, h: 0.86, margin: 0, valign: "middle",
    fontFace: SANS, fontSize: 11, color: IVORY, lineSpacingMultiple: 1.24 });
    s.addNotes("20%割れは回転23.7ヶ月超。ネガティブ複合では13.5ヶ月が分岐点で想定レンジ内。");
}
...
// ═══════════════════════════════════════════════════════════ 15 条件
{
  ...
  const left = [
    ...
    [td("損益の配分"), td("SPC税前利益を折半（投資家50：WineBank50）。WineBankは現金拠出なし・期中報酬も成功報酬も課さない", { align: "left", color: GOLD_L, bold: true })],
  ...
  const right = [
    ...
    [td("想定リターン"), td("投資家利回り35.0%（在庫回転12ヶ月・定常年間）。5年通算の年平均利回りは27.9%", { align: "left", color: GOLD_L, bold: true })],
  ...
  s.addText("半期解約を前提とすると常時1〜3割を現金または即時換金可能な在庫で保有する必要があり、稼働率が下がって利回りも下がる（稼働率95%→85%で投資家利回りは35.0%→31.2%）。当初3年ロックアップ・4年目以降は年度ごとの解約という設計により、現金保有を5%に抑え、総額の95%をワインに充当できる。", {
...
// ═══════════════════════════════════════════════════════════ 17 まとめ
{
  ...
  const merits = [
    ...
    ["収益が二階建てで、片方が崩れても成立する",
     "流通価格差（回転に対するリターン）とワインの値上がり（在庫に対するリターン）。価格が横ばいでも投資家利回り28.4%、回転が18ヶ月に延びても24.8%を確保する。"],
    ["他社が再現できない調達力",
    ...
    ["運営者と利害が完全に一致している",
     "期中報酬も成功報酬も取らず、報酬はSPC税前利益の折半に一本化されている。加えてデット2億円の金利と保証責任もWineBankが負担する。投資家が儲からなければWineBankも儲からない。"],
  ];
  let my = 1.72;
  merits.forEach((m, i) => {
  ...
  const checks = [
    ["想定利回り20%（実績）に対して",
     "在庫回転12ヶ月を主線とし、投資家利回り35.0%。想定レンジ9〜18ヶ月では24.8〜45.2%となり、いずれも貴社基準を上回る。",
     "流動性の高い銘柄を組み入れることで回転12ヶ月を実現する。回転期間を運用KPIとして四半期報告する。"],
    ["解約条件に対して",
     "運用期間5年・当初3年ロックアップ・4年目以降は年度ごとの解約。",
     "この設計により現金保有を5%に抑え、総額の95%をワインに充当できる。半期解約を前提とすると稼働率が下がり35.0%→31.2%に低下する。"],
    ["リスク分散に対して",
