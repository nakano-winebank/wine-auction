...
a=[('2026/08 既存事業 売上',57500000,'貴社想定：8-9月の2か月で110〜120百万円。ここでは中央値115百万円を折半'),
   ...
   ('2026/08 販管費',25624504,'2026/04-07実績の月平均25,624,504円'),
...
up=[('① 私募ファンド3号',500000000,70000000,'9月末に組成完了。粗利60百万円＋販売手数料10百万円'),
...
p.cell(UT,5,'粗利105,000,000円。原価は売上−粗利＝525,000,000円').font=RD
n=UT+2
bar(p,n,'【4】商品在庫の見通し　※FY2027の期首在庫に直結します',4)
iv=[('2026/07末 商品（実績）',721162328,False,'添付の貸借対照表 月次推移より'),
    ('2026/08-09 商品仕入（想定）',0,True,'★8-9月の仕入予定額をご入力ください'),
    ('2026/08-09 売上原価（出庫）',None,False,'②月次推移表の8月・9月の売上原価 合計'),
    ('2026/09末 商品（見込）',None,False,'期首＋仕入−出庫')]
...
IV0=n+1; IV_BUY=n+2; IV_OUT=n+3; IV_END=n+4
p.cell(IV_END+2,2,'※私募ファンドの原価525百万円を在庫から出庫する場合、9月末の商品在庫は約196百万円まで減ります。').font=RD
p.cell(IV_END+3,2,'※従来のFY2027モデルは期首在庫420百万円を前提としていました。ここが変わると来期の仕入計画と資金繰りに影響します。').font=RD
P=lambda r: f"'③前提'!$C${r}"
...
R_F3=r+2; row(R_F3,'＋私募ファンド3号',None,0,f'={P(U0)}',ind=1)
R_F4=r+3; row(R_F4,'＋私募ファンド4号',None,0,f'={P(U0+1)}',ind=1)
...
R_CF=r+2
row(R_CF,'＋私募ファンド 原価',None,0,f'=({P(U0)}-{PD(U0)})+({P(U0+1)}-{PD(U0+1)})',ind=1)
R_CT=r+3
m.cell(R_CT,2,'売上原価 合計').font=SB
for i in range(12):
    L=get_column_letter(3+i)
    c=m.cell(R_CT,3+i,f'={L}{R_CE}+{L}{R_CF}'); c.number_format=YEN; c.font=SB; c.fill=TF; c.border=TB
...
m.cell(R_CO+4,2,'※上乗せ4件はすべて2026/09に計上。粗利105,000,000円・原価525,000,000円。').font=RD
m.cell(R_CO+5,2,'※「7月末までの修正（戻し）」は販管費の減少として8月に計上。勘定科目は要確認。').font=RD
m.freeze_panes='C5'
MM=lambda r,c='O': f"'②月次推移表'!${c}${r}"
p.cell(IV_OUT,3,f"=SUM('②月次推移表'!M{R_CT}:N{R_CT})").number_format=YEN
p.cell(IV_END,3,f'=C{IV0}+C{IV_BUY}-C{IV_OUT}').number_format=YEN
p.cell(IV_END,3).fill=OR_
...
br=[('10か月実績 営業利益（2025/10-2026/07）',f'=SUM(\'②月次推移表\'!C{R_OP}:L{R_OP})+{P(ADJ)}',
    ...
    ('上乗せ4件の粗利',f'={PD(UT)}','私募3号70＋4号5＋M&A仲介20＋コンサル料10＝105百万円'),
    ('FY2026 着地 営業利益',None,'')]
...
nt=[('8-9月の売上レンジは着地に影響しない','営業利益トントンの前提では、売上が110百万円でも120百万円でも営業利益は変わりません。'
    ...
    ('商品在庫','2026/07末の商品は721,162,328円。私募2本の原価525百万円を在庫から出庫する場合、'
     '9月末は約196百万円まで減ります。従来のFY2027モデルの期首在庫420百万円とは差があります。'),
    ('消費税','課税売上高が1,190百万円前後まで増えるため、FY2026分の確定納付額が従来想定より増えます。'
     'FY2027の資金繰りに影響するため、翌期PLの作成時に織り込みます。')]
