...
/* 2 エグゼクティブサマリー */
{
  ...
  const items=[
    ...
    ['粗利率は保守的に置いた','既存事業29.5%（過去2期の加重平均）。4-6月実績の51.5%は使いません。']
  ];
  items.forEach((it,i)=>{
    const y=1.62+i*1.22;
  ...
  s.addText('＋8.8',{x:8.6,y:2.2,w:3.8,h:0.85,fontFace:HF,fontSize:42,bold:true,color:GOLD,align:'center',margin:0});
  s.addText('百万円',{x:8.6,y:3.03,w:3.8,h:0.3,fontFace:BF,fontSize:11,color:'D9D7D2',align:'center',margin:0});
  s.addText('粗利率を保守的に置いても\n営業損益は黒字転換します',{x:8.6,y:3.42,w:3.8,h:0.62,fontFace:BF,fontSize:11.5,color:WHT,align:'center',margin:0});
  s.addShape(p.ShapeType.line,{x:8.85,y:4.24,w:3.3,h:0,line:{color:'6E6E6E',width:1}});
  s.addText('期中 最低現預金',{x:8.6,y:4.4,w:3.8,h:0.3,fontFace:BF,fontSize:11,color:'D9D7D2',align:'center',margin:0});
  s.addText('＋14.6百万円',{x:8.6,y:4.7,w:3.8,h:0.6,fontFace:HF,fontSize:26,bold:true,color:GOLD,align:'center',margin:0});
  s.addText('売上730.5百万円（前期・今期並み）\nワイン仕入419.1百万円を維持',{x:8.6,y:5.36,w:3.8,h:0.7,fontFace:BF,fontSize:11,color:WHT,align:'center',margin:0});
  foot(s,'出典：事業計画202608（銀行様）全社シート月次実績、決算報告書 第54期、2026年7月度 取締役会資料');
  s.addNotes('プランDは売上を前期・今期並みに置き、粗利率を保守的に落とした前提。営業＋8.8百万円、経常▲6.2百万円だが、減価償却43.7百万円があるため資金は回る。');
}
...
/* 9 4プラン比較 */
{
  const s=base('4つのプラン － Dを本線とします','売上は前期・今期並み、粗利率は保守的。上振れを織り込まない前提で組み立てています。');
  const data=[
    ['売上高（上乗せ含む）','389.1','521.6','680.8','730.5'],
    ['既存事業の粗利率','51.5%','51.5%','51.5%','29.5%'],
    ['売上総利益 合計','200.6','333.1','415.2','308.9'],
    ['総合粗利率','51.5%','63.9%','61.0%','42.3%'],
    ['販管費','300.2','300.2','300.2','300.2'],
    ['営業利益','▲99.6','＋32.9','＋115.0','＋8.8'],
    ['経常利益','▲114.6','＋17.9','＋100.0','▲6.2'],
    ['期中 最低現預金','－','－','－','＋14.6']
  ];
  table(s,0.65,1.66,[{t:'（百万円）',w:3.1},{t:'A. 横ばいのみ',w:2.15,a:'right'},{t:'B. ＋上乗せ132.5',w:2.15,a:'right'},
    {t:'C. 経常利益1億円',w:2.15,a:'right'},{t:'D. 保守シナリオ',w:2.25,a:'right'}],data,{rh:0.40,hi:[5,6,7],hs:9});
  card(s,0.65,5.45,5.85,1.42,BERRY);
  s.addText('A〜C は粗利率51.5%を前提',{x:0.95,y:5.62,w:5.25,h:0.3,fontFace:BF,fontSize:11,bold:true,color:GOLD,margin:0});
  s.addText('2026年4-6月の実績値。ただしこの水準は大口の在庫販売が入らなかった3か月のもので、通期で維持できる保証はありません。',
    {x:0.95,y:5.93,w:5.25,h:0.85,fontFace:BF,fontSize:10.5,color:WHT,valign:'top',margin:0});
  card(s,6.8,5.45,5.85,1.42,CREAM);
  s.addText('D は粗利率29.5%を前提',{x:7.1,y:5.62,w:5.25,h:0.3,fontFace:BF,fontSize:11,bold:true,color:INK,margin:0});
  s.addText('FY2025・FY2026の加重平均。大口の在庫販売を含む実績なので、より実態に近い保守値です。損益は均衡ですが資金は回ります。',
...
/* 10 ワイン在庫戦略 */
{
  ...
  const cards=[['期首在庫（2026/09末）','420.0百万円',INK],['FY2027 仕入','419.1百万円',BERRY],
               ['FY2027 売上原価（出庫）','421.6百万円',INK],['期末在庫','417.5百万円',GOLD]];
  cards.forEach((c,i)=>{
    const x=0.65+i*3.09;
  ...
  s.addText('仕入419.1百万円と売上原価421.6百万円がほぼ見合うため、在庫は420.0→417.5百万円と横ばい。仕入を維持しながら在庫に資金を固定しません。',
...
/* 11 FY2027 月次推移表 */
{
  const s=base('FY2027 月次推移表 － プランD','売上は前期・今期並み、既存事業の粗利率は29.5%（保守）。上乗せ案件132.5百万円は原価なし。単位：百万円');
  const MO=['26/10','26/11','26/12','27/01','27/02','27/03','27/04','27/05','27/06','27/07','27/08','27/09'];
  const gEx=176420431/12, adv=92500000/12, sga=25013422, noe=1250000;
  const inc=i=>i===5?20000000:0, cr=i=>(i===5||i===11)?10000000:0;
  const defs=[
    ['売上高（既存事業）',i=>598035360/12],['売上原価',i=>-(598035360-176420431)/12],
  ...
  [['売上高 合計（上乗せ含む）','730.5百万円'],['通期 営業利益','＋8.8百万円'],['通期 経常利益','▲6.2百万円']].forEach((c,i)=>{
    const x=1.0+i*3.95;
  ...
  s.addNotes('通常月は経常▲3.9百万円だが、3月にM&A仲介と新規クルーザー事業30百万円が乗り通期はほぼ均衡。減価償却43.7百万円があるため資金は回る。');
}

/* 12 月次資金繰り */
{
  const s=base('FY2027 月次資金繰り － プランD','ワイン仕入419.1百万円を維持。売上原価421.6百万円とほぼ見合うため在庫は横ばいで、資金が固定されません。単位：百万円（税込）');
  const MO=['26/10','26/11','26/12','27/01','27/02','27/03','27/04','27/05','27/06','27/07','27/08','27/09'];
  ...
  const cs=[['A. プランD（本線）','＋41.4','＋14.6'],['B. 仕入を売上原価に連動','＋38.6','＋11.8'],
            ['C. 一時収益44百万円が未入金','▲2.6','▲29.4'],['D. 売上が前期・今期並みに届かない','▲153.4','▲180.2']];
...
/* 15 蓋然性 ★ */
{
  const s=base('プランDの蓋然性 － 過去実績が裏付けています','売上は前期・今期と同水準。上振れを織り込まず、粗利率も過去実績の平均に落としています。');
  const data=[
    ['FY2024 実績','512.5','203.7','39.8%','－'],
    ['FY2025 実績','752.9','201.9','26.8%','97.0%'],
    ['FY2026 見込','708.2','229.1','32.4%','103.2%'],
    ['プランD（FY2027）','730.5','308.9','42.3%','100.3%']
  ];
  ...
  s.addText('プランDの730.5百万円は、FY2025実績752.9とFY2026見込708.2のちょうど平均。新たな成長を前提にせず、過去2期に到達した売上規模をそのまま置いています。',
    {x:0.95,y:4.7,w:5.25,h:0.95,fontFace:BF,fontSize:11,color:WHT,valign:'top',margin:0});
  card(s,6.8,4.15,5.85,1.55,CREAM);
  s.addText('粗利率は保守的に据え置き',{x:7.1,y:4.33,w:5.25,h:0.32,fontFace:BF,fontSize:12,bold:true,color:INK,margin:0});
  s.addText('既存事業は29.5%（FY2025・FY2026の加重平均）に据え置き。4-6月実績の51.5%は使いません。総合42.3%になるのは上乗せ案件132.5百万円が原価なしのためです。',
    {x:7.1,y:4.7,w:5.25,h:0.95,fontFace:BF,fontSize:11,color:INK,valign:'top',margin:0});
  card(s,0.65,5.9,11.99,0.85,CREAM);
  s.addText('既存事業の粗利率が4-6月実績の51.5%まで戻れば、同じ売上730.5百万円で経常利益は約185百万円。プランDはその上振れを一切織り込んでいません。',
    {x:1.0,y:5.9,w:11.3,h:0.85,fontFace:BF,fontSize:12,bold:true,color:BERRY,valign:'middle',margin:0});
  foot(s,'プランDの事業粗利308.9百万円＝既存事業176.4（粗利率29.5%）＋上乗せ案件132.5（原価なし）。出典：事業計画202608（銀行様）、決算報告書 第54期。');
...
/* 16 まとめ */
{
  ...
  const pts=[
    ...
    ['売上は過去2期と同水準・粗利率は保守','売上730.5百万円は2期平均そのもの。粗利率29.5%も過去2期の加重平均です。']
  ];
  pts.forEach((t,i)=>{
    const y=1.75+i*1.12;
