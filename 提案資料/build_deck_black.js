...
const BG="000000", PANEL="17111A", PANEL2="261A24", BURG="7B1E3A",
...
/* 5 STRUCTURE — 2契約 */{
  ...
  [["① マスターリース（動産賃貸借）","賃料：保管・保険・管理の役務提供",
    ["会員が所有するワインをWineBankが賃借する","賃料は保管・保険・真正性管理の役務で充当する","マイルは賃料ではない（LEGAL 参照）"],GOLD_L],
  ...
  s.addText("不動産のファンドと同じ形です。マスターリース会社が借りて運用し、AM会社が運用報酬とディスポジションフィーを取る。",
  ...
  s.addText("マイルは賃料ではありません。値上がりの75%は会員のもの。当社の取り分は25%だけ。",
    t({x:M+0.45,y:5.98,w:CW-0.9,h:0.42,fontSize:17,bold:true,color:GOLD_L}));
  s.addNotes("純粋サブリース（値上がり全取り）は買戻し義務を伴い収益認識で詰む。2契約に分けることで、法務・会計・マーケの3つが同時に立つ。／2026年9月の弁護士意見書により、マイルを賃貸借の対価（賃料）として発行すると資金決済法の前払式支払手段に該当し得るため、賃料は役務提供で充当し、マイルは管理手数料に対する無償付与として整理する。");
}
...
/* 11 TWO TRACKS + 年次精算 */{
  ...
  [["運用型（推奨・基本）","賃貸借 ＋ 成功報酬25%",["ワインは当社が賃借（賃料は役務で充当）","マイルは管理手数料に対する無償付与","値上がりの75%は会員のもの。出口は自由"],PANEL2,GOLD],
...
/* 12 MILE STRUCTURE */{
  ...
  s.addNotes("マイルの有効期限は無償付与であれば自由に設定できる（資金決済法の前払式支払手段に該当しないため）。JALマイル36ヶ月・楽天1年と同様。ただし消費者契約法10条により、事前告知なく一方的に失効させる条項は無効となり得るため、失効の30日前予告を規約とシステムの両方に組み込む。");
}
...
/* 19 RISK */{
  const s=base("LEGAL","法務確認済み：実施できます",
    "岩田合同法律事務所より2026年9月に回答を取得。「法律上の問題なく実施できる」との結論です。");
  [["資金決済法","無償付与なら該当しない",
    "無償で発行する限り前払式支払手段には該当せず、届出も供託も不要です。ただしマイルを賃貸借の『賃料』として発行すると対価性が生じ該当し得るため、管理手数料に対するおまけとしての無償付与に整理を統一します。"],
   ["出資法・銀行法","そもそも該当しない",
    "預かるのはワインであり金銭ではないため、『預金』『預り金』の4要件を満たしません。当初懸念していた「年率と言わない」という制約は不要になりました。"],
  ...
  s.addNotes("岩田合同法律事務所（大櫛・関口・國本）2026年9月回答。残る宿題は3点：①マイルを有償で販売する導線は本意見書の対象外であり、対価性が生じるため前払式支払手段に該当する。実装する場合は別途照会が必要 ②賃料を役務提供で充当する場合の契約書文言 ③マイル無償付与の課税整理（税理士）。");
}
...
/* 22 NEXT */{
  ...
  [["WineBank CLUBの実数","ランク別の年会費・特典・提携店数。BLACKの特典内容がSIGNATUREの購買動機を決めます。"],
   ...
   ["残る法務・税務の宿題","①マイル利用規約の整備（経産省ガイドラインの6項目）②賃料を役務で充当する契約書文言 ③無償付与マイルの課税整理（税理士）④古物競りあっせん業者の届出要否"]
  ].forEach((v,i)=>{
...
pres.writeFile({fileName:"/home/user/wine-auction/提案資料/WineBank_ワインマイル経済圏構想_黒背景版.pptx"})
