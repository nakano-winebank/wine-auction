...
T(s,'合算後の自己資本比率は表面38.6%、のれん控除後11.8%と試算されます（次ページ）。MBO完了後に合併した場合はそれぞれ48.6%・26.3%です。',
  {x:0.7,y:6.5,w:11.9,h:0.4,fontSize:9.5,color:C.MUT});
s.addNotes('この一枚が「金融機関的にどちらが良いか」への回答。資金の行き先が全て。');

/* 6 統合BS */
s=slide(); light(s);
head(s,'比較 ③','合算すると、バランスシートはこうなります');
card(s,0.7,1.68,4.05,4.6,C.TINT_B,C.TINT_BL);
T(s,'スペースマーケット社の財務',{x:1.0,y:1.88,w:3.5,h:0.3,fontSize:13.5,bold:true,color:C.BLUE});
T(s,'2025年12月期（2025年12月31日現在）',{x:1.0,y:2.2,w:3.5,h:0.26,fontSize:9.5,color:C.MUT});
[['総資産','37.39億',C.INK],['純資産','10.07億',C.INK],['自己資本比率','24.6%',C.INK],
 ['現金及び現金同等物','11.56億',C.POS],['固定負債','7.91億',C.INK],
 ['売上高','25.67億',C.INK],['営業利益','2.47億',C.POS]].forEach((r,i)=>{
  const y=2.58+i*0.44;
  T(s,r[0],{x:1.0,y:y+0.03,w:2.2,h:0.3,fontSize:10.5,color:C.INK2});
  T(s,r[1],{x:3.0,y:y,w:1.45,h:0.34,fontSize:14,bold:true,color:r[2],align:'right'});
});
T(s,'2026年12月期 会社予想　売上 32.44億／営業利益 2.90億',
  {x:1.0,y:5.72,w:3.45,h:0.42,fontSize:10,bold:true,color:C.BLUE,lineSpacingMultiple:1.15});
tbl(s,[
 [{text:'合算後',options:TH},{text:'いま合併',options:THC},
  {text:'MBO完了後',options:Object.assign({align:'center'},TH,{fill:C.ACC})}],
 [{text:'のれん',options:{bold:true}},{text:'22.09億',options:{align:'center'}},{text:'22.95億',options:{align:'center'}}],
 [{text:'総資産',options:{bold:true}},{text:'72.64億',options:{align:'center'}},{text:'75.97億',options:{align:'center'}}],
 [{text:'純資産',options:{bold:true}},{text:'28.07億',options:{align:'center'}},{text:'36.92億',options:{align:'center',bold:true}}],
 [{text:'自己資本比率（表面）',options:{bold:true}},{text:'38.6%',options:{align:'center'}},{text:'48.6%',options:{align:'center',bold:true}}],
 [{text:'のれん控除後の自己資本',options:{bold:true,fill:'F3E7EA'}},{text:'5.98億',options:{align:'center',bold:true,color:C.NEG,fill:'F3E7EA'}},{text:'13.97億',options:{align:'center',bold:true,color:C.POS,fill:'F3E7EA'}}],
 [{text:'のれん控除後の比率',options:{bold:true,fill:'F3E7EA'}},{text:'11.8%',options:{align:'center',bold:true,color:C.NEG,fill:'F3E7EA'}},{text:'26.3%',options:{align:'center',bold:true,color:C.POS,fill:'F3E7EA'}}],
 [{text:'合算 営業利益',options:{bold:true}},{text:'4.90億',options:{align:'center'}},{text:'4.90億',options:{align:'center'}}],
 [{text:'のれん償却後（20年）',options:{bold:true}},{text:'3.80億（△23%）',options:{align:'center'}},{text:'3.75億（△23%）',options:{align:'center'}}],
 [{text:'合算 現預金',options:{bold:true}},{text:'11.96億',options:{align:'center'}},{text:'13.75億',options:{align:'center',bold:true}}]],
 {x:5.05,y:1.68,w:7.55,colW:[3.05,2.25,2.25],rowH:0.40,fontSize:11});
card(s,5.05,5.98,7.55,0.62,C.TINT_A,C.TINT_AL);
T(s,'銀行はのれんを自己資本から控除して見ます。表面38.6%でも実質11.8%です。',
  {x:5.35,y:6.12,w:7.0,h:0.34,fontSize:12.5,bold:true,color:C.ACC});
T(s,'スペースマーケット社の数値は同社決算短信の公表値です（検索結果経由で取得、PDF原本は未確認）。有利子負債の内訳は未入手のため、合算の有利子負債は算出していません。',
  {x:0.7,y:6.72,w:11.9,h:0.3,fontSize:9,color:C.MUT});
s.addNotes('のれん控除後11.8%が銀行の見方。MBO後なら26.3%。この14.5ptの差が答え。');
...
[['01','重松氏の意向を、まず書面にする','会長就任の時期、譲渡する株数、価格の考え方。この3点を覚書にしていただきます。口頭の前提のままでは、銀行に資金計画として持ち込めません。',C.ACC],
 ...
 ['03','条件面の論点は、いまのうちに投げておく','評価額18億円の根拠、基準株価を3ヶ月平均とすること、のれん償却の負担配分。あわせて同社の有利子負債の内訳と直近四半期の貸借対照表の開示を求めます。',C.ACC]]
.forEach((r,i)=>{
  const y=1.72+i*1.35;
